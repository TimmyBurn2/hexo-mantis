"""The actor-lag watchdog.

Invariant: `learner_step − actor_ckpt_step > N` fires exit 45 when armed, and emits ONE loud
...
def _watchdog(tmp_path, *, spec, sink=None, exit_codes=None):
    """Build a watchdog whose staleness fire is structurally silenced by a deadline of 0.
...
def test_build_run_safety_wires_actor_lag_from_monitor_config(tmp_path, monkeypatch) -> None:
    """Prove the built watchdog carries the monitor config's threshold and both callables live.

    The fire's detail echoes the injected values, read back from the real JSONL sink.
    """
...
# The census of ActorLagSpec inputs that must carry no default lives here, once, and is
# parametrized one item per name so each name dies alone.
@pytest.mark.parametrize(
    ...
    [
        ...
        # Measured: a defaulted `monitor_cfg` fell back to a bare `MonitorConfig()` and a lag
        # of 10 000 over a threshold of 100 produced `exit codes []` — the reading stays live.
        ("monitor_cfg",
...
def test_lag_check_reads_callables_live_not_captured_values(tmp_path) -> None:
    """Prove the check reads its callables at poll time: advancing the learner mid-run fires.

    A value captured at ctor or arm would read a frozen delta forever and never fire.
    """
    codes: list[int] = []
    learner = {"v": 3}
