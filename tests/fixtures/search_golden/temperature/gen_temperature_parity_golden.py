...
Pins the contract that the Python ``get_temperature(mode="training")`` path and
the Rust ``compute_move_temperature`` path produce the SAME within-game
temperature for the same (ply, threshold, temp_min) — so the two formulas + the
ply→compound-turn clock cannot silently diverge.
...
The Rust twin `crates/mantis-search/tests/temperature_parity_golden.rs` reads the
emitted CSV and asserts its impl reproduces `expected_tau` within 1e-6 (Rust is
f32, ~1 ULP below tol); the Python eval/bot path checks the same fixture.

Regenerate:  python gen_temperature_parity_golden.py
"""
...
# (threshold_compound, temp_min) — control(OFF) + the 3 probe arms + a legacy
# cosine + a wide-window case.
