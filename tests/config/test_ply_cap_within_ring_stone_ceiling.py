...
#: Any census member: each row varies only the ply cap, so the base only has to be a real mint.
_VEHICLE = production_configs(pathlib.Path(__file__).resolve().parents[2])[0]
...
def test_a_cap_at_the_ceiling_validates(smoke_run_config) -> None:
    ...
    config = smoke_run_config(_VEHICLE, selfplay={"max_game_moves": max_stones()})
...
def test_a_cap_one_past_the_ceiling_REDS_AT_MINT(smoke_run_config) -> None:
    ...
    with pytest.raises(ValidationError, match="exceeds the HEXG ring"):
        smoke_run_config(_VEHICLE, selfplay={"max_game_moves": max_stones() + 1})
...
def test_the_refusal_names_both_operands_and_the_way_out(smoke_run_config) -> None:
    ...
    with pytest.raises(ValidationError) as excinfo:
        smoke_run_config(_VEHICLE, selfplay={"max_game_moves": 4096})
