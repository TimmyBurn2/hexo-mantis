...
# Each value names a REAL reader. The eval.gate leaves are not threaded into the
...
CONSUMER_REGISTRY = {
    ...
    "train.ply_cap_abort.rate": "resolve_ply_cap_abort -> StepCoordinatorConfig.ply_cap_abort -> step.py::_run_ply_cap_gate -> check_ply_cap_attractor (the ply-cap attractor HALT, R352(c); null is OFF)",
...
def test_every_registry_string_names_symbols_that_exist() -> None:
    ...
    dead = {key: toks for key, text in CONSUMER_REGISTRY.items()
            if (toks := unresolved_tokens(text, defined))}
...
def test_the_resolver_bites_on_a_dead_symbol() -> None:
    defined = defined_names()
    assert unresolved_tokens("resolve_monitor_config -> no_such_function_xyz", defined) \
