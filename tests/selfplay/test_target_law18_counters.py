"""⊕ Phase T (TARGET INTEGRITY) — CTR, PYTHON SURFACE LEG: the
...
`gridls_zero_policy_rows` was the third and LEFT: it counted §3.5 zero-row
...
(the idle-lever posture). NOTE (recorded): the
`getattr(..., 0)` legacy-wheel default in `runner_stats` means surface visibility is
NOT a producer proof — that burden stays on the Rust legs.
