...
and `ragged_policy_ce`/`GnnNet.forward_batch` consume both as required, no-default
...
def test_ragged_policy_ce_and_forward_batch_require_masks_no_default() -> None:
    """The "unconstructible" half of A4: a caller cannot construct a valid call without
    supplying `legal_offsets`/`legal_mask` — no accidental omit-and-fall-back path."""
    ...
    assert fwd_params["legal_mask"].default is inspect.Parameter.empty
