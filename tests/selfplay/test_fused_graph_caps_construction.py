...
def test_fg6_08_the_round_spec_carries_the_caps_across_the_process_seam() -> None:
    ...
    assert "fused_graph_caps" in RoundSpec.__dataclass_fields__, (
        ...
        "runs unbounded")
    caps = FusedGraphCapsSpec(max_fused_edges=4_500_000, max_fused_nodes=170_000)
    spec = RoundSpec(**_round_spec_base(), fused_graph_caps=caps, leaf_batch_size=1, c_visit=50.0, c_scale=1.0, q_rescale=True, search_kind="puct", gumbel_m=16, max_plies=128, leaf_build_threads=1, concurrency=1,
...
def test_fg6_08_a_grid_round_carries_none_across_the_same_seam() -> None:
    ...
    spec = RoundSpec(**_round_spec_base(), fused_graph_caps=None, leaf_batch_size=1, c_visit=50.0, c_scale=1.0, q_rescale=True, search_kind="puct", gumbel_m=16, max_plies=128, leaf_build_threads=1, concurrency=1,
