...
#:
#: The other four were TEST-ONLY LAW-08 findings — carried to the device and read by no production
#: path — and R297(c)/R298(d) resolved them ONE COMMIT PER FIELD. Each re-expressed onto the
#: surface that genuinely consumes it: `legal_mask`'s agreement rows onto the gather (gaining an
#: explicit uniqueness assertion the old scatter only implied), and `policy_dst_slot` /
#: `window_center` / `current_player` onto the WIRE arrays, which the bridge and
#: `verify_edge_geometry` actually read. The wire keeps every one of them.
