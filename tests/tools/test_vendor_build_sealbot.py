"""The sealbot BUILD step is reproducible FROM THE TREE, not from a record.

The REFUSAL rows are the load-bearing ones: a `.so` compiled from an unpinned tree, or from a
tree whose patch never applied, loads and plays — it just plays a different engine, at a depth
receipt no downstream oracle can distinguish. Each precondition gets its own row and exit code.

Driven against a SYNTHETIC root rather than the repo root, where the real vendored tree is
present and built: the script resolves its root from its own location and uses only relative
paths, so a root holding `tools/` + `vendor/` exercises the same code with no real state
touched.
"""
...
#: Any 40-hex literal. The script must carry NONE — `vendor/pins.toml` is the one sha authority.
...
def _fake_pin(root: Path, *, sha: str, march_native: bool, win_threshold: bool) -> None:
    ...
    # Rewrite the pin to the sha this synthetic tree landed at, unless the row wants DRIFT.
...
def test_the_build_script_is_tracked_and_executable() -> None:
    """A repair that is not in a commit is a repair that lives in a record."""
...
def test_the_script_carries_no_sha_literal_and_reads_the_pin_instead() -> None:
    """ONE sha authority: a hardcoded sha builds the right source until the pin moves."""
...
def test_the_build_never_re_syncs_the_PROJECT_environment() -> None:
    """`uv run` inside the repo tree re-syncs the project venv, which swaps the CUDA torch
    wheel for the CPU one under every later measurement.

    Asserted on the COMMAND, because the outcome is invisible until something else is measured.
    """
    body = _SCRIPT.read_text(encoding="utf-8")
...
def test_it_refuses_when_the_fetched_tree_drifted_from_the_pin(tmp_path: Path) -> None:
    """The row that stops a build over an unpinned engine: a drifted tree was moved after fetch."""
    root = _synthetic_root(tmp_path)
...
def test_it_refuses_when_the_pinned_patch_is_not_applied(
    ...
    """BOTH patch hunks, separately: the fetch skips a renamed or untracked patch SILENTLY,
    so the build is where that is caught."""
    root = _synthetic_root(tmp_path)
