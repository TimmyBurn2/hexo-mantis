"""The collected-test-count gate must BITE — its producer test.

>300 justify (R8): one gate's producer suite over one synthetic-repo fixture, where each arm's
PASS case is the near-miss of another arm's FAIL case.

The two defects: the gate resolved its ref as `origin/main`, a branch this repo has never had,
and fell back to the WORKING TREE's own floor, so lowering the floor passed trivially; and the
count came off a pipeline discarding pytest's status and stderr, so an interrupted collection
passed the gate whose purpose is to notice tests went missing. Every case drives the SCRIPT in
a throwaway git repo, since a re-implemented comparison could drift from what it certifies.
"""
...
#: Identity for the synthetic repos: a machine with no global git config would otherwise fail
...
def _seed(repo: Path, ref_floor: int) -> None:
    """Build a repo whose `dev` branch carries `ref_floor`, with HEAD moved off `dev`."""
...
def make_repo(
    ...
    ``origin`` only the remote ref; ``local`` only the local one; ``both`` with DIFFERENT floors
    so precedence is observable; ``fetchable`` a remote but neither ref; ``none`` bootstrap.
    """
    repo = tmp_path / name
    ...
    if ref == "both":
        # A local `dev` that DISAGREES with the remote, so the winning arm is visible.
    ...
    if ref in ("fetchable", "none"):
        # A default `fetch-depth: 1` checkout leaves exactly this shape.
        subprocess.run(
...
#: Fake collections as (status, stdout). The first two are VERBATIM what this tree printed on
#: 2026-09-02 with and without a planted import break; the third swallows the status.
...
def run_gate_measuring(
    ...
    """Drive the gate's REAL measuring path with a fake `pytest` standing in for collection —
    a shell script, not a monkeypatch, since only a real child has a status and output."""
...
def test_a_lowered_floor_fails(tmp_path: Path) -> None:
    """A lowered floor fails: the count clears both floors, so the edit down is the ONLY
    fault — and it is how a lost suite was laundered into a green gate."""
...
def test_lost_tests_plus_a_lowered_floor_reports_both(tmp_path: Path) -> None:
    """A suite disappears and the floor is edited to match: both arms must report."""
...
def test_origin_dev_outranks_a_local_dev(tmp_path: Path) -> None:
    """A stale local `dev` must never outrank the fetched one."""
...
def test_a_shallow_checkout_recovers_the_ref_by_fetching(tmp_path: Path) -> None:
    """A shallow checkout has neither ref, and without the fetch the ONE run that matters
    would silently take the bootstrap arm."""
...
def test_the_bootstrap_arm_is_loud_and_says_what_it_did_not_check(tmp_path: Path) -> None:
    """The bootstrap arm compares, passes, and SAYS the property is off — silence would leave
    its CI log indistinguishable from a real comparison."""
...
def _blinded(tmp_path: Path) -> Path:
    """Return the gate with `verdict` neutered — the mutation the self-test must catch."""
...
def test_the_self_test_runs_before_the_real_verdict_not_after(tmp_path: Path) -> None:
    """The self-test runs before the verdict, or it guards nothing in CI."""
    repo = make_repo(tmp_path, 100, 100, ref="origin")
...
def test_a_broken_collection_is_refused_even_though_it_printed_a_count(tmp_path: Path) -> None:
    """A broken collection is refused though it printed a count: 4414 of 4414 collected with
    one file gone is 4414 of 4428."""
    repo = make_repo(tmp_path, ref_floor=4396, tree_floor=4396, ref="origin")
...
def test_the_failing_module_is_named_in_the_refusal(tmp_path: Path) -> None:
    """The refusal carries pytest's own error block, or the reader re-runs by hand."""
    repo = make_repo(tmp_path, ref_floor=4396, tree_floor=4396, ref="origin")
...
def test_a_swallowed_status_is_caught_by_the_summary_line(tmp_path: Path) -> None:
    """The summary-line arm is not redundant: the original bug WAS a lost exit status."""
    repo = make_repo(tmp_path, ref_floor=4396, tree_floor=4396, ref="origin")
...
def test_no_ci_step_injects_a_count() -> None:
    """The injection seams are test-only: in a CI `run:` body they would fake the gate."""
...
def test_the_committed_tree_is_green_against_the_real_ref() -> None:
    """The committed tree is green against the real ref, with its own floor as the injected
    count so no real collection is paid for."""
