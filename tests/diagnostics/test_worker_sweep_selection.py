...
def test_the_knee_picks_the_smallest_rung_within_the_band_not_the_fastest() -> None:
    ...
    selection = ws.select_knee(rows, knee_pct=95.0, metric="moves_per_min", noise_floor_rel_std=0.0)
...
def test_the_ranking_metric_is_PINNED_and_cannot_be_swapped_by_a_caller() -> None:
    ...
    assert ws.select_knee(rows, knee_pct=95.0, metric="moves_per_min", noise_floor_rel_std=0.0)["picked"] == 4
    with pytest.raises(ValueError, match="pre-registered"):
        ws.select_knee(rows, knee_pct=95.0, metric="games_per_min", noise_floor_rel_std=0.0)
...
def test_the_knee_percent_is_taken_from_SOURCE_in_both_modes() -> None:
    ...
    for edited in (60.0, 100.0, 94.9):
        with pytest.raises(ValueError, match="R309"):
            ws.select_knee(rows, knee_pct=edited, metric="moves_per_min", noise_floor_rel_std=0.0)
...
def test_a_rung_row_that_smuggles_n_workers_1_is_refused_not_picked() -> None:
    ...
    with pytest.raises(ValueError, match="REJECTS"):
        ws.select_knee([_row(1, 5.0)], knee_pct=95.0, metric="moves_per_min", noise_floor_rel_std=0.0)
...
def test_a_non_finite_ranking_value_is_refused(value: float) -> None:
    ...
    with pytest.raises(ValueError, match="NaN"):
        ws.select_knee([_row(2, 30.0), _row(4, value)], knee_pct=95.0, metric="moves_per_min", noise_floor_rel_std=0.0)
...
def test_an_identically_zero_ranking_column_is_refused_not_picked_from() -> None:
    with pytest.raises(ValueError, match="cannot order the ladder"):
        ws.select_knee([_row(2, 0.0), _row(4, 0.0)], knee_pct=95.0, metric="moves_per_min", noise_floor_rel_std=0.0)
...
def test_an_empty_report_says_so_instead_of_talking_about_memory() -> None:
    ...
    selection = ws.select_knee([], knee_pct=95.0, metric="moves_per_min", noise_floor_rel_std=0.0)
    assert selection["picked"] is None
...
def test_rc_is_zero_when_a_pick_was_made() -> None:
    ...
    report = {"rungs": rows, "selection": ws.select_knee(rows, knee_pct=95.0,
                                                         metric="moves_per_min", noise_floor_rel_std=0.0)}
...
def test_the_selection_screen_prints_every_input_the_rule_ran_on(capsys) -> None:
    ...
    for needle in ("knee_pct=95", "passing rungs", "best passing", "threshold =",
                   "R317(d) noise floor", "at or above adjusted threshold", "PICK = 4"):
...
def _noise_floor_report(tmp_path: Path, rel_std: float = 0.0) -> Path:
    """R317(d): a minimal `--noise-floor` report `--select-only` can read `rel_std` off."""
    path = tmp_path / "noise_floor.json"
    path.write_text(json.dumps({"tool": ws.TOOL, "mode": "noise_floor",
                                "noise_floor": {"rel_std": rel_std}}), encoding="utf-8")
    return path
...
def test_select_only_re_derives_the_same_pick_from_a_written_report(tmp_path: Path,
    ...
    nf_path = _noise_floor_report(tmp_path)
    assert ws.main(["--select-only", str(path), "--noise-floor-report", str(nf_path)]) == 0
    text = capsys.readouterr().out
