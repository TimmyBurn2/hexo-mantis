...
def test_disagreeing_flat_string_and_nested_identity_raise_the_named_error() -> None:
    ...
    msg = str(exc.value)
    assert "gnn_axis_v1" in msg and "gnn_axis_r8" in msg, (
...
def test_disagreeing_flat_mapping_and_nested_identity_raise_the_named_error() -> None:
    with pytest.raises(EncodingDeclarationConflictError):
        resolve_from_config({"encoding": {"version": "gnn_axis_r8"},
...
def test_agreeing_dual_shape_resolves_like_single_shape(cfg) -> None:
    assert resolve_from_config(cfg).name == resolve_from_config({"encoding": "gnn_axis_r8"}).name


def test_single_shapes_resolve_unchanged() -> None:
    assert resolve_from_config({"encoding": "gnn_axis_v1"}).name == "gnn_axis_v1"
...
def test_anchor_lifter_is_a_veneer_absence_is_none_conflict_raises() -> None:
    ...
    assert _resolve_declared_encoding({"encoding": {"version": "gnn_axis_r8"}}) == "gnn_axis_r8"
    ...
    with pytest.raises(EncodingDeclarationConflictError):
        _resolve_declared_encoding({"encoding": "gnn_axis_v1", "identity": {"encoding": "gnn_axis_r8"}})
...
def test_anchor_private_name_form_is_dead() -> None:
    ...
    narrowing, pinned)."""
    assert _resolve_declared_encoding({"encoding": {"name": "gnn_axis_r8"}}) is None
