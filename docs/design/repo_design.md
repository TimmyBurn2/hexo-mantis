...
3. **Why a key and not a constant.** `consec` counts consecutive OBSERVATIONS and an
   observation is appended per gate run, so the sampling stride IS the gate's time constant.
   Running the gates per step without a knob would have re-scaled every armed abort by
   `log_interval` — run5's armed draw-rate row (0.25 / 25000 / 50) would silently have meant
   50 STEPS instead of 50 boundaries, a 1000× change to a pre-registered armed value. Armed
   values are operator-only, so the cadence had to become an explicit, separately-settable
   fact rather than a side effect of a logging knob.
