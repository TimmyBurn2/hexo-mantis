...
|---|---|---|---|---|
| 01 | AMENDED | C | (−1 793 `wc -l`+span, not proposed) | not spent: upstream half of the live bootstrap_encode contract; R247 standing in RULINGS.md; KEEP |
| 02 | AMENDED | C | (−268/−33, not proposed) | `proofs` is falsified.md F-53's named instrument → KEEP; test-floor move −3 |
| 03 | CONFIRMED | C | (≤ −124, bound only) | 0 producers in src+crates for all 7 inputs; operator choice on pre-R362 records |
| 04 | CONFIRMED | C | (−227, KEEP) | manifest.toml + 2 Rust files cite it as provenance |
| 05 | CONFIRMED | C | 0 | 23 rows; rustc on line 17; 4 files say 28 |
| 06 | CONFIRMED | C | 0 | both AMENDMENTs present in repo_design |
| 07 | CONFIRMED | B | ≈ −51 (spans 63 re-derived) | 5 tools + 2 tests copies |
| 08 | CONFIRMED | B | −35 −26 tests (hexlogic `wc -l` re-derived) | engine exports HEX_AXES/WIN_LENGTH/find_winning_line verified in the bridge |
| 09 | CONFIRMED | B | ≈ −13 (not re-derived) | fork declared in board.js line 1 |
| 10 | CONFIRMED | B | ≈ −9 | identical regex, _shard_paths span 8 |
| 11 | CONFIRMED | B | ≈ −14 | same predicate at both readers; spans 19/16 |
| 12 | AMENDED | A (12a) / PENDING-PROBE (12b) | −3 (12a probe); 12b net 0 unprobed | 12b torch-bound |
| 13 | CONFIRMED | C | ≈ −8 | 6 sites; PZ owns checkpoints.py loader + build_net |
| 14 | CONFIRMED | C | ≈ −20 | RoundSpec( only in pipeline.py + strength_frontier.py |
| 15 | AMENDED | A | −8 (probe) | Δ was −6; the separating blank lines go too |
| 16 | CONFIRMED | C | ≈ −6 | PZ-4 minter |
| 17 | CONFIRMED | C | −4 | event_manifest.md cites the shim symbol by name |
| 18 | CONFIRMED | C | −1 | CARD-LADDER-RUNG names the README |

### Per-finding notes
S-A-TOOLS-2-01 — AMENDED: `grep -n -i audit src/mantis/data/bootstrap_encode.py` -> "Encode an audited move-list bootstrap corpus", `_require_record` = "The audit's contract v2, re-checked at the point of USE", the record-file rule "which is the audit's own rule"; `git grep -l bootstrap_encode -- src` -> reached from src/mantis/train/pretrain/graph_route.py; `sed -n 3713,3716p docs/governance/RULINGS.md` -> R247 "Status: standing" (the canonical register, not only archive/; the scout's path grep missed it because R247/R279 name the audit by function, not by path), and R-line 3832 promotes the bootstrap path to MINT-CRITICAL. The tool is the certify stage of a live capability pipeline (a new corpus drop or re-certification goes through it), so ONE-SHOT does not hold: class -> KEEP, lane C unchanged. Collection re-derived: `pytest --collect-only -m '' test_audit_bootstrap_corpus.py test_probe1.py` -> 42 (38 + 4); floor file reads 4862; tier_declaration.txt has no row for either file.
S-A-TOOLS-2-02 — AMENDED: `git grep -n "probe1" -- docs/governance/falsified.md` -> F-53's evidence cell names "`tools/probe1.py proofs`" as the instrument of a falsification; a register row naming the tool is a reproducibility record (LAW-02/LAW-05 re-validation needs it) -> `proofs` + rings.py::{full_arm_rows,reconstructed,target_argmax} KEEP. Also docs/design/measurements/PROBE1_2026-09-21.md names each of the six subcommands as its reading's "Instrument:"; STATE's PROBE-1 READ line says the instrument is "pinned by tests/tools/test_probe1.py" — taking 3 of its 4 tests is a test-floor move (−3). Scout's netread citation re-derived (RUN10_PREREG KL row). Successors re-derived: ring_audit.py::per_mr_rows exists with `one_hot_share_full_mr<k>`/`tail_only_full`; src/mantis/train/heldout.py exists. Witness overstated: `grep -n "def test_" tests/tools/test_probe1.py` -> the one kept test pins netread's calibration/kl reductions; no test pins `spread`. Import map re-derived: full_arm_rows/reconstructed/target_argmax are used only by proofs.py and the test, so the scout's delete list is internally consistent.
S-A-TOOLS-2-03 — CONFIRMED: per-name `git grep -c -F <name> -- src crates` summed -> 0 for eval_channel_health, eval_rung_activated/graduated/skipped, eval_ladder_zero_game_round, wr_sealbot, eval_ladder_state; AST read of strength.py::primary_rung/sealbot_readings_present/rung_series -> all three key only on channel-health, the ladder file or wr_sealbot, so no post-R362 rung can reach them. The RUNG_RETIRED_NOTE gap is CLAUDE.md's "drawn as a stated gap, never as a zero" rule at work, which is why the call belongs to the operator (lane C). Δ is an upper bound, not a net.
S-A-TOOLS-2-04 — CONFIRMED: `git grep -n -F gen_mctx_parity_fixtures` -> manifest.toml provenance comment, parity_tests.rs module doc, mctx_parity.rs doc + panic message; `wc -l` 227. KEEP stands.
S-A-TOOLS-2-05 — CONFIRMED: `grep -c '^\[floor\.' tools/bench_floors.toml` -> 23; `grep -n '^rustc'` -> line 17 against rust-toolchain.toml's `bench_floors.toml:11`; `git grep -E "\b28\b.{0,20}(floor|bench)"` outside docs/governance -> CLAUDE.md, Makefile, mise.toml, rust-toolchain.toml.
S-A-TOOLS-2-06 — CONFIRMED: CLAUDE.md "Deliberately absent" still says ONE exception; repo_design.md carries "AMENDMENT — R352(g), VIEWER-1" and "AMENDMENT — R363, ANALYZER-1".
S-A-TOOLS-2-07 — CONFIRMED: `git grep -n submodule_search_locations -- tools tests` -> 5 tools + tests/tools/{conftest,test_probe1}.py; own AST spans 13/13/13/12/12 = 63.
S-A-TOOLS-2-08 — CONFIRMED: `git grep -E "HEX_AXES|WIN_LENGTH|find_winning_line" -- crates/mantis-bridge/src` -> encoding.rs `m.add("HEX_AXES")`/`m.add("WIN_LENGTH")`, board.rs `find_winning_line`; reader.py already imports mantis.monitor.game_record, so the engine is in reach. Lane B (4 tests go).
S-A-TOOLS-2-09 — CONFIRMED: `head -1 tools/analyzer/web/board.js` -> "forked from tools/viewer/html.py". Δ not re-derived here.
S-A-TOOLS-2-10 — CONFIRMED: `git grep -F 'games_(?P<run>' -- src tools` -> byte-identical regex at game_record.py and viewer/reader.py; _shard_paths span 8.
S-A-TOOLS-2-11 — CONFIRMED: `git grep -F '"shard_closed"' -- src tools` -> writer + mirror_receipts.py and mirror_pull.py reading with the same `record == "shard_closed" and shard` predicate; spans 19/16.
S-A-TOOLS-2-12 — AMENDED: probe (worktree scratchpad/wt/S-A-TOOLS-2-rv*, edit script in scratchpad): strix_follower `import hashlib` + `_sha256` dropped, `from mantis.util.hashing import sha256_file` added as its own import group (ruff `I` is selected, so it cannot take hashlib's slot); select_balanced_book swapped in place. `git diff --stat` -> strix_follower 3+/6− (net −3, not −4), select_balanced_book 2+/2− (net 0). Import ok; `pytest --collect-only -m ''` -> 2289 collected / 167 errors, ERROR lines identical to HEAD; ruff clean; gate 15 green; gate 10 ran clean; cargo check ok; tests/tools/test_strix_follower.py per-test outcomes identical to HEAD (11 passed including the sidecar `checkpoint_sha256 == hashlib.sha256(...)` pin; 1 FAILED at HEAD too). 12b is PENDING-PROBE: the tool imports mantis.arena -> mantis.util.device -> torch, and test_select_balanced_book.py errors at collection at HEAD (torch-bound). strix_follower stays over 300 lines (309), so its R8 header stays.
S-A-TOOLS-2-13 — CONFIRMED: `git grep -E "metadata\.arch is None" -- tools` -> 6 sites (analyzer/engines, bench_server, ladder/backends, probe1/nets, probe1/swa, strength_frontier); PZ.md lists train/checkpoints.py's loader and model/build.py::build_net, so a shared helper lands in PZ.
S-A-TOOLS-2-14 — CONFIRMED: `git grep -F "RoundSpec(" -- src tools` -> pipeline.py + strength_frontier.py only; PZ.md lists aggregate.py::pair_units (gate pair statistics) and pipeline.py.
S-A-TOOLS-2-15 — AMENDED (Δ only): hunted callers beyond the scout. `git grep -E "challenges\(|list_bots|api/bot/challenges|api/bots\b|LadderClient"` outside the module -> session.py/ladder_bot.py hold the client and call neither method; BUILD.md does `/api/bots?online=1` by hand; CARD-LADDER-RUNG (CARDS.md) plans neither endpoint. Probe: both methods and their separating blank lines removed -> `git diff --stat` client.py −8 (the scout's −6 counted spans only). Collection identical (2289/167, same ERROR lines); import ok; ruff clean; gate 15/10 green; cargo check ok; ladder_client/session/bot per-test outcomes identical to HEAD. Only test_endpoint_strings_live_only_in_the_client_module runs without torch, and it passes; the other 33 are torch-bound at HEAD too. That test is the one pin an endpoint removal could red, so this is CONFIRMED green, with the caveat stated. client.py is 163 lines, so no R8 header is involved.
S-A-TOOLS-2-16 — CONFIRMED: `awk 'NR>=214&&NR<=256' tools/mint_config.py` -> both loops end in the same `_delta_line` / `except HeaderRenderError` / `parent[leaf] = new` / `delta_lines.append` tail; PZ-4.
S-A-TOOLS-2-17 — CONFIRMED: `git grep -E "SIZE_CAP_BYTES|MIRROR_LAG_WARN_BUNDLES"` -> shim readers only in tests/tools/test_run_dashboard.py, and `grep -o` finds `tools/run_dashboard.py::MIRROR_LAG_WARN_BUNDLES` in docs/contracts/event_manifest.md.
S-A-TOOLS-2-18 — CONFIRMED: `wc -l` 1; CARDS.md CARD-LADDER-RUNG names "the README" under tools/ladder/vps/.
DEFECTS re-checked: probe1/cli.py `--run-id default="run8"` present; select_balanced_book.py cites configs/run7.yaml (the file exists, so this is an R10 naming defect, not a dangling path); hardcode_scan.py names `n_chain_planes`, which `git grep` does not find under src/mantis/encoding. HANDOFF bench count re-derived: `[[bench]]` 2+1+1+2 = 6.

### Tally: raised 18 | confirmed 14 | amended 4 (01, 02, 12, 15) | refuted 0 | pending 0 whole (12b partial, torch-bound) | architect 0
