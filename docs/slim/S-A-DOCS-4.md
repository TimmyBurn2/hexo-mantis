...
|---|---|---|---|---|
| S-A-DOCS-4-01 | AMENDED | C | 0 | (a)(b)(d) hold; (c) REFUTED: RULINGS.md is titled "R23 to R367" and "From R346" says WHEN the entries became canonical, not which ones |
| S-A-DOCS-4-02 | CONFIRMED | C | 0 | attestation is `[provenance].rustc`; line 11 is the R346(f) deletion note; 23 `[floor.*]` tables |
| S-A-DOCS-4-03 | CONFIRMED | A | 0 (mise.toml 1+/1-) | probe green |
| S-A-DOCS-4-04 | CONFIRMED | A | -1 | probe green; registry.toml keeps `text: unset` |
| S-A-DOCS-4-05 | CONFIRMED | A | 0 (4+/4- in the comments) | 45 rows, include_str! at golden_tests.rs::GOLDEN_BITS, 8 `.sh` |
| S-A-DOCS-4-06 | AMENDED | A (was C) | -3 | a comment-only edit with no protected contact; the tie to 07 is sequencing, not protection; moot if 07 lands |
| S-A-DOCS-4-07 | ARCHITECT | C | n/a | readers listed below; the LAW-15 ground is misattributed |
| S-A-DOCS-4-08 | ARCHITECT | C | n/a (-558 by `wc -l` if deleted) | readers listed below |
| S-A-DOCS-4-09 | AMENDED | A | -5 (README 3+/8-) | STATUS rewritten in 3 lines pointing at STATE.md, and the comment plus its blank line dropped; the scout's -3 is the floor |
| S-A-DOCS-4-10 | AMENDED | A | -6 (Cargo.toml 1+/7-) | also drops the `#` separator the F-52 paragraph would orphan |
| S-A-DOCS-4-11 | CONFIRMED | B | -4 (not probed, lane B) | repo_design names the profile twice (§7 profile list and the profiling line) |
| S-A-DOCS-4-12 | CONFIRMED | C | ≤ -202 | counts re-derived; gate 14's comment_lint does NOT scan toml (the scout's open assumption is now verified) |

### Per-finding notes
S-A-DOCS-4-01 — AMENDED: `sed -n 1,6p docs/governance/RULINGS.md` -> "# RULINGS — R23 to R367 … From R346 these entries are **canonical**"; `grep -n '^### R23 ' RULINGS.md` -> present. So "the canonical ruling entries, R23 onward" is right and sub-item (c) falls. (a) `tomllib` -> `floor` has 23 tables; CLAUDE.md "all 28". (b) LAWS.md's head: archive/laws.md holds "the pre-R346 wording"; LAWS.md:LAW-10 says "DELETED by R347(d)" and archive/laws.md:LAW-10 gives the live text. (d) repo_design has "AMENDMENT — R352(g), VIEWER-1 … ADMITTED" and "AMENDMENT — R363, ANALYZER-1 … ADMITTED". Three stale statements, lane C (operator's file).
S-A-DOCS-4-02 — CONFIRMED: `tomllib.load(bench_floors.toml)` -> keys [provenance, floor], 23 floor tables, `provenance.rustc = "rustc 1.97.1 …"` (file line 17). Line 11 reads "the rustc/CPU attestation above still holds". A floor "row" = one `[floor.<bench_id>]` table. PZ-6 in the scratchpad PZ.md also says "the 28 floors"; the dispatcher's map carries the same stale count.
S-A-DOCS-4-03 — CONFIRMED: `grep -n "28 bench" mise.toml` -> the parenthetical. Probe worktree scratchpad/wt/rev-docs4-A (all lane-A edits batched): `import mantis` ok; collect `-m ''` -> "2289 tests collected, 167 errors" (= baseline); `cargo check --workspace --all-targets --locked` -> Finished; nearest tests (test_line_endings, test_fixtures_manifest, test_vendor_pins_{sealbot,strix}, test_vendor_fetch_idempotent, test_law13_no_target_cpu…, test_bridge_cache_keys, test_one_artifact_pin_authority, test_bench_floors) -> 47 passed; gate 10 rc 0; comment_lint GREEN.
S-A-DOCS-4-04 — CONFIRMED: for each non-comment rule, `git ls-files -- ":(glob)<pat>" | wc -l` -> manifests.toml 0, registry.toml 1, tests/fixtures/** 64. In the probe, `git check-attr` -> registry.toml and tests/fixtures/manifest.toml "text: unset", tools/vendor_fetch.sh "text: set". test_line_endings passes.
S-A-DOCS-4-05 — CONFIRMED: `tomllib` -> manifest `required` = 45 rows; `grep -n include_str golden_tests.rs` -> :75 (`GOLDEN_BITS`); `git ls-files '*.sh'` -> 8, all with `#!/usr/bin/env bash`; registry/mod.rs:25 `REGISTRY_TOML` is right today. Line 3's "33 of 65" is a dated measurement ("measured on this repo before this file existed"). It is history, not drift, so it stays.
S-A-DOCS-4-06 — AMENDED to lane A: `sed -n 12,20p vendor/pins.toml` -> the parenthetical splits "so the" / "checkout is sha-driven". Removing it is a pure comment edit, and test_vendor_pins_sealbot passes in the probe. It is independent of 07, but superseded if 07 lands.
S-A-DOCS-4-07 — ARCHITECT: may the sealbot pin and patch (and so the adapter chain) go after R362(c), or must an external fixed-depth instrument stay buildable? Readers by name (`git grep -l -i sealbot -- ':!docs'`): Makefile `vendor.sealbot` + `.PHONY`; CLAUDE.md Build & test (`make vendor.sealbot`); tools/vendor_build_sealbot.sh; tools/vendor_fetch.sh (generic over `[pins.*]`, applies `patch`). The BOX procedure `make vendor` (STATE / CALLERS §5) therefore fetches sealbot and applies sealbot.patch today. The scout's "never vendor.sealbot" is true of the target name but misses this reader. src: bots/{resolve,sealbot,strix}.py (strix imports `find_vendor_root`), config/resolve/nsims.py, plus retired-name tolerance in eval/{pipeline,rounds,worker}.py, train/checkpoints.py, train/coordinator/{drain,step}.py, monitor/{rules.py,producer_manifest.yaml}, data/corpus_analysis.py. tools: dashboard/{hero,strength,tier2,tier3}.py, strength_frontier.py, select_balanced_book.py, ci_gates/tier_declaration.txt (3 rows). 41 tests/ files, including 2 value_probes jsonl fixtures and tests/test_meta_ci.py's target set. Amendments: LAWS.md LAW-15 does not name sealbot (`grep -c -i sealbot LAWS.md archive/laws.md` -> 0/0). The "reproducible bar IS depth" wording is src/mantis/bots/resolve.py::_NO_DEPTH_REASON. tests/tools/test_vendor_fetch_idempotent.py does NOT name sealbot (absent from the grep). R362's text deletes the rung and its "five consumers"; it does not name the adapter or the pin.
S-A-DOCS-4-08 — ARCHITECT (question as the scout wrote it). `git grep -l -E "run[678]\.yaml"`: run6 -> src/mantis/config/armed_aborts.py, src/mantis/eval/pipeline.py, tools/strength_frontier.py, 55 tests, CARDS.md (1 line); run7 -> tools/select_balanced_book.py, 3 tests, STATE.md (4 lines, including the "Minted values — `configs/run7.yaml`" section); run8 -> 6 tests, STATE.md (6 lines, including the deploy-seam and derivation records). The census readers (`production_configs|EXEMPT_CONFIGS`) are src/mantis/config/{census,armed_aborts}.py, tools/ci_gates/preflight_mint.py and 13 test files. No PZ-3 frozen file names run6-8. CLAUDE.md names none. `wc -l` -> 176+190+192 = 558.
S-A-DOCS-4-09 — AMENDED Δ: `grep -n -E "run8|STOPPED" STATE.md` -> "run8 STOPPED at 55 170", "run7 STOPPED … at 83 482", so "no production training run has completed" and "run6 identity is being minted" are stale. The F-52 comment also says "five-crate", while the workspace has 6 members (README line 28 already says "six-crate"). Probe: gate 10 rc 0 on the rewritten README.
S-A-DOCS-4-10 — AMENDED Δ: `git grep -n is_multiple_of -- crates` -> 9 sites in 7 files, none in board.rs. Probe: `cargo check --workspace --all-targets --locked` green.
S-A-DOCS-4-11 — CONFIRMED lane B: `git grep -n -E "profile[ =]+profiling|target/profiling|CARGO_PROFILE"` -> only PERF_INVESTIGATION_2026-09-11.md, which used `CARGO_PROFILE_RELEASE_DEBUG=true`. The one recorded profiling session bypassed the profile, which strengthens "unused". Removing it still needs a repo_design amendment (R9).
S-A-DOCS-4-12 — CONFIRMED: `grep -c -E '^\s*#'` / `wc -l` is identical for all 8 files. `tools/ci_gates/comment_lint.py` scopes `rel.endswith((".py", ".rs"))`, so no floor moves.

### Missed by the scout
NEW-1 | TEST (handoff T3) | B: tests/config/test_armed_abort_cadence.py binds `RUN5 = REPO_ROOT / "configs" / "run6.yaml"`. A runN symbol breaks R10, and it names the wrong run. It rides with 08.

### Tally: raised 12 | confirmed 6 | amended 4 | refuted 0 | pending 0 | architect 2
