...
| the stamped `config` is schema-validated at load as PROVENANCE: a leaf the schema grew after the stamp, or a dotted path it RETIRED (`mantis.config.retired.RETIRED_PATHS`, the one authority `tools/config_diff.py` reads too: `search`, and since R362(c) `eval.ladder`, `eval.sealbot_model_sims`, `eval.rung_concurrency` and the nine `monitor.wr_*` leaves that every run7/run8 stamp carries), is logged `checkpoint_config_predates_schema` and TOLERATED, and a resume drops the retired paths from the config the Trainer carries (`resume_retired_paths_dropped`, the stamp untouched) so its first save does not refuse; any other validation error refuses the load with the payload untouched (run7's 23 829 stop checkpoint loaded under `missing=['eval.max_plies', 'eval.gate.sequential']`) | mantis.train.checkpoints (`_validate_stamped_config`), mantis.config.retired (`RETIRED_PATHS`) |
