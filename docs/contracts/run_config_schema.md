...
Ten top-level fields; **184 leaf key-paths** under the walker that descends nested blocks
...
| every schema leaf key has a live consumer (184-entry bijection, LAW-08), in two independently-maintained copies | tests/config/test_every_key_has_consumer.py + tests/config/test_every_key_has_consumer_p2.py |
