...
| `_draw_rate_evidence_bar_is_reachable` | `RunConfig` | `train.draw_rate_abort.N_pool_min` <= `DRAW_RATE_WINDOW * selfplay.n_workers`, the measured ceiling of the pooled window sum; this is the bound that replaced the retired `min_samples: le=DRAW_RATE_WINDOW` pin |
