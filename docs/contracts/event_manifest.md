...
| `trainer_step.policy_target_entropy`, `trainer_step.policy_kl_target_vs_prior` | `Trainer.train_step_from_graph_batch`, from `train.losses.ragged_policy_ce_and_target_entropy` — R350(b)(iv)'s first line: H(target) over the step's policy rows (the reconstructed sparse tail inside it), reduced exactly as the CE, and `CE - H` = KL(target ‖ this model's policy) — how far the search's target sits from the prior it was searched from | `tests/train/test_sparse_gumbel_row_target.py::test_the_real_graph_trainer_step_publishes_the_kl_line` |
