REDACTED DERIVATIVE — 7 fragment(s) replaced by stable placeholders under rule 7. Canonical: mantis-migration/plan/rulings_register.md @ a7cb442, regenerated 2026-09-08.
...
# R323 — architect ruling, ARCH-ERA session, 2026-08-31 (SEAM-B2 RATIFIED in full with the withdrawn W-L2 correction COMMENDED; candidate D's config key DISPOSED to run6's mint and the arch-width gap parked for the ablation era; the box lane ORDERED as three items — the opponent pool repaired because a promotion gate is only as meaningful as its opponents, eval.strength_floor proven with its arming values PROPOSED and RESERVED to the operator, and F-816-25 executed or scoped so the BC-pretrain path stands unblocked) [INLINE]

**Provenance: [INLINE], authored.** Text originates in the ARCH-ERA architect session under R303(d), forwarded by the operator into this execution session's prompt rather than as a file — so the landing session CREATED the ruling's one canonical home, `PACKET_R323_SITTING4_PREP.md` §1, and no competing text exists under `plan/` (`grep -rn 'R323' plan/` returned nothing at all before that file was written). Byte-diff on append per R306(a) (de-marked, whitespace-normalized, line-by-line; extraction by `awk` over the canonical home, never retyped; extraction and diff under `set -o pipefail`, R313(c)). **The forwarded text carried the placeholder `R{next}`**, substituted to `R323` after verifying the head is R322 — census `R23–R322, 293 sections / 293 distinct / 0 duplicates, excluded set 53`, missing-in-range `{24, 29, 32, 33, 227, 228, 267}`; `--stamp` reported `STAMP OK: stamp v3.28 == §8 last entry v3.28`; `--self-test` all four controls fire; last register header `R322`, no `R323` heading present. **The LIVE-placeholder check `grep -rn '^> *R{next}' plan/` returns ZERO hits, and that is the CORRECT reading for this landing's shape rather than a missed check.** R321's correction expects exactly one hit because a packet is normally authored upstream and substituted at landing; here the text arrived in a prompt and the landing session authored the canonical home AFTER substituting, so no un-landed placeholder exists under `plan/` to find. What stands in its place is stronger: the forwarded text was transcribed a SECOND time, independently, with `R{next}` intact, substituted mechanically by `sed`, and diffed de-marked and whitespace-normalized against the landed entry — IDENTICAL, which is what establishes that the substitution is the only edit. That edit was applied to the canonical home as well as to this entry, so the two are byte-identical (`diff` of the two `awk`-extracted blocks is empty); the `R323` occurrences in the packet's filename and preamble are REFERENCES, not canonical text, and were left untouched.

**The ruling's referents re-derived at point of use before landing, not transcribed from the leg record.** **Clause (a).** The eight-row repair is `1659fb6`, and the ratchet is DELETED rather than widened: `tests/model/conformance/test_config_partition_shared_vs_arch_scoped.py:184` reads `DECLARED_RED_ROWS: frozenset[tuple[str, str]] = frozenset()`, with the vacuity guard an empty expectation needs driven against a synthetic row at `:439`. The defect the repair itself introduced was caught by an EXISTING oracle — `tests/config/test_armed_abort_manifest.py:589::test_a_minted_config_survives_a_dump_revalidate_round_trip` — and fixed at the serializer, `model_serializer(mode="wrap")` on `RunConfig`, `src/mantis/config/schema/core.py:441`. Candidate D's mechanism is `16ab35a`: `ARCH_KINDS` (`src/mantis/model/arch.py:122`), `ARCH_KINDS_BY_REPRESENTATION` (`:131`), `INCUMBENT_ARCH_KIND = {"grid": "CnnArch", "graph": "GnnArch"}` (`:153`), and `select_arch(spec, config, *, arch_kind)` keyword-only with NO default (`:207`). The archive is `e75b004`, and ValueHead's transitive burial is a FIELD of the golden rather than a claim in prose: `tests/fixtures/model_graves/hexonet_grave_v1.json` carries `param_count` **914**, `forward_digest` `2827d5a4…`, `state_dict_digest` `a151eb27…` and `value_head_buried_with_it`. Both codecs are `f049ef0` — `lambda_return_targets` and `scalar_to_hl_gauss` in `src/mantis/model/value_targets.py` — and their unarmedness is asserted STRUCTURALLY, an `ast` import census over `src/` and `tools/` at `tests/model/conformance/test_value_target_codecs_land_unarmed.py:360-380`, not a grep (R296(f), structure-not-text). The census's pre-registered branch is `plan/research/SCOUT_2026-08-30_CANDIDATES.md:845` — *"the LARGE branch fires: `S-CODEBOOK` STAYS at rank 6 and its memory-system risk"* — written onto the card in place per R322(d). The withdrawn W-L2 correction is `plan/SEAM_B2_EXIT.md` §4.1. **Clause (b)'s** adopted disposition is `SEAM_B2_EXIT.md` §2's shape 3, which that section already marked **Recommended**; the parked finding is the same section's structural one, that `arch_from_spec_and_config` reads ten arch hyperparameters and not one is a live `RunConfig` leaf. **Clause (c)(i)'s** subject is `plan/RECAL_SITTING3_RECORD_2026-08-28.md` §9, re-stated at `plan/EVAL_CHANNEL_1_RECORD.md` §5.4 caveat 2; the per-rung mechanism is derived at `src/mantis/bots/resolve.py` and `src/mantis/bots/sealbot.py` by the executing session, never inferred from either record. **Clause (c)(ii)'s `~50 s`** is `plan/EVAL_CHANNEL_1_RECORD.md` §5.3 item 4, which attributes the figure to *"§5.2's arithmetic there"* — a pointer to `EVAL_POSTURE_OPTIONS.md`, not a measurement in that record; the ruling's own instruction is to read it at that producer, and the leg record states what was found. **Clause (c)(iii)'s** F-816-25 is `plan/ADJUDICATION_QUEUE.md:4118`, already RULED by **R296(b)** (`plan/rulings_register.md:5570`): *"the shadowing flags are removed or become config-only"*, sequenced *"BEFORE any BC-pretrain execution"*.

> R323 — (a) SEAM-B2 is RATIFIED in full: the eight-row repair with its
> ratchet deleted by the fix and its own introduced defect caught by an
> existing oracle and fixed at the serializer; candidate D's mechanism with the
> incumbent pinned against every minted config; the archive verdicts (HeXONet +
> ValueHead archived with goldens and fence proven; HexTacToeNet kept with
> consumers named); both codecs landed UNARMED with witnesses passing and
> unarmedness asserted structurally; the census's pre-registered branch firing
> (S-CODEBOOK stays rank 6). The withdrawn W-L2 correction is COMMENDED — a
> wrong fix withdrawn is the discipline working.
> (b) Candidate D DISPOSITION ADOPTED: the arch-selector key enters production
> configs only as a minted row at run6's mint. The RunConfig shape finding (no
> width/depth leaf) is RECORDED and parked for the ablation era.
> (c) BOX-LANE ORDER: (i) the opponent pool is repaired — sitting 3 recorded
> sealbot, kraken and strix all UNRESOLVABLE at HEAD; a promotion gate is only
> as meaningful as its opponents, and sitting 4 does not forward against an
> empty ladder. (ii) eval.strength_floor is implemented and proven with a
> planted break; its arming VALUES are proposed from EVAL-CHANNEL-1's measured
> data, pre-stated in the record, and RESERVED to the operator's sitting-4
> forwarding. (iii) F-816-25's fix executes if field-rulable, else its measured
> scope returns — the BC-pretrain path stands unblocked pending the operator's
> bootstrap adjudication.
> ROUTE: land this, census, ACTIVE next version; execute §3–§5; one plain exit
> screen.

# R324 — architect ruling, ARCH-ERA session, 2026-08-31 (SITTING4-PREP-1 RATIFIED with sitting 3's stated sealbot cause CORRECTED on the record; the ~50 s figure corrected at its three carry sites; the sealbot repair made REPRODUCIBLE from the tree; the strength_floor values PROPOSED not armed and their deciding ground ordered MEASURED; NIGHTRUN scope adopted as five severable legs — hygiene, perf tranche-2, the floor lane, bootstrap CAPABILITY unarmed, box measurements) [INLINE]

**Provenance: [INLINE], authored.** Text originates in the ARCH-ERA architect session under R303(d), forwarded by the operator into this NIGHTRUN-1 execution session's prompt rather than as a file — so the landing session CREATED the ruling's one canonical home, `PACKET_R324_NIGHTRUN_1.md` §1, and no competing text exists under `plan/` (`grep -rn 'R324' plan/` returned nothing at all before that file was written). Byte-diff on append per R306(a) (de-marked, whitespace-normalized, line-by-line; extraction by `awk` over the canonical home, never retyped; extraction and diff under `set -o pipefail`, R313(c)). **The forwarded text carried the placeholder `R{next}`**, substituted to `R324` after verifying the head is R323 — census `R23–R323, 294 sections / 294 distinct / 0 duplicates, excluded set 53`, missing-in-range `{24, 29, 32, 33, 227, 228, 267}`; `--stamp` reported `STAMP OK: stamp v3.29 == §8 last entry v3.29`; `--self-test` all four controls fire; last register header `R323`, no `R324` heading present. **The LIVE-placeholder check `grep -rn '^> *R{next}' plan/` returns ZERO hits, and that is the CORRECT reading for this landing's shape** — the same shape as R323's and for the same reason, stated rather than copied: the text arrived in a prompt, so the landing session authored the canonical home AFTER substituting and no un-landed placeholder exists under `plan/` to find. What stands in its place: the forwarded text was transcribed a SECOND time, independently, with `R{next}` intact, substituted mechanically by `sed`, and diffed de-marked and whitespace-normalized against the landed entry — IDENTICAL, 26 lines (`wc -l` over the extracted block, derived at the check and not carried from the packet). That substitution is the ONLY edit to the canonical text and was applied to the canonical home as well as to this entry, so the two are byte-identical; the `R324` occurrences in the packet's filename and preamble are REFERENCES, not canonical text, and were left untouched.

**The ruling's referents re-derived at point of use before landing, not transcribed from the leg record — and one of them was found ASSERTED rather than shown, and is now driven.** **Clause (a)'s "four biting planted breaks"** is the referent in question. `plan/SITTING4_PREP1_EXIT.md` states it ONCE, in its §0 summary table, and never enumerates it: §2.3 describes three test ROWS and two kinds of evidence, which is a different object. So the landing session PLANTED FOUR BREAKS AND DROVE THEM against `src/mantis/eval/worker.py::run_round` PHASE 0 at `b59c91e`, control `3 passed` before and after — (B1) the early return deleted, (B2) the verdict forced to `passed=True` after the production rule decided, (B3) the probe run unconditionally, (B4) the `floor_probe` phase mark dropped. **All four bite**; B3 reds the disarmed-identity row alone and B4 reds two rows, which is the phase series being a shared instrument rather than a per-row assertion. Enumerated in `plan/NIGHTRUN1_EVIDENCE/leg0_planted_breaks_strength_floor.md`. The clause is ratified on a derivation, not on the summary line. **Clause (a)'s remaining referents:** `sealbot`'s mechanism marker is `BUILD_ABSENT_MARKER = "sealbot extension not built"` at `src/mantis/bots/sealbot.py:84`, raised at `:189`; R139's per-rung grounds are `src/mantis/bots/resolve.py:45-48` (`kraken` "weights not cleanly accessible", `strix` "actively changing"); **ANNOTATION 10** is `plan/rulings_register.md:5289` and narrows R257's `[r8]` fence rather than lifting it. **`eta_min`'s 50× divergence is re-derived at both ends**, not transcribed: `configs/run5.yaml:111` mints `eta_min: 0.0005` and the deleted code-side literal was `1e-5` — 50×, and it was the ONE site of the eleven that F-816-25's filed census never named. **Sitting 3's stated cause is corrected on the record** because `vendor/pins.toml:12` carries a full `[pins.sealbot]` table with sha `c94749c21c16c3b072fff6da49762dd5f92f3986`; the literal `[pins]` above it is an empty PARENT table, and the pin is its child. **Clause (b)'s three carry sites** are enumerated by search, not by memory: `plan/EVAL_POSTURE_OPTIONS.md` §5.2 (the PRODUCER's arithmetic), `plan/EVAL_CHANNEL_1_RECORD.md` §5.3 item 4, and `plan/SEAM_V1_DESIGN.md` "Stage 1". `plan/RULINGS_ACTIVE.md:1880` is NOT a carry — it instructs a reader to read the figure at its producer — and is correctly left standing. **Clause (c)'s premise held only in half:** `tools/vendor_fetch.sh` + `make vendor` already fetch AND patch at the pinned sha; the step that lived only in a record was the COMPILE, so what lands is the build half with its preconditions refused rather than a re-implementation of the fetch. **Strix's adapter DOES NOT EXIST at contact** — `src/mantis/bots/` holds `random_bot.py`, `sealbot.py`, `resolve.py` and `protocol.py` and no strix module, and `vendor/pins.toml` carries no strix pin — so clause (c)'s conditional takes its ELSE arm by construction: the scope is reported and the loud-skip stands. **Clause (d)'s reason-branch site** is `src/mantis/train/coordinator/step.py:1137`, whose reason vocabulary was the closed two-value set `"eval_round_broken" if broken is not None else "wr_sealbot_absent"`; the gap the leg record did NOT name is that `build_round_result` never carried the floor verdict at all, so no branch on it could have fired. **Clause (e)'s perf referents** are `plan/research/PERF_TRANCHE1_RESULTS.md` §2.2 (`collate_h2d` ~81 % not-the-copy), §2.1 (the fuse's invariant 16.8 ms/pop of source-side reads), and §8 item 1 (the 7.2 % pre-control-vs-ledger discrepancy, 3.152 vs 3.396 ms/sim); the floor-lane cards are `plan/research/SCOUT_2026-08-30_CANDIDATES.md:639` (`S-INCR-GRAPH`, whose pre-registered falsifier is F-19's own inequality) and `plan/research/PERF_RESEARCH.md:685` (C1, the fused sorted-CSR GINE kernel, against the measured 19.6 ns/edge). **Clause (e)'s bootstrap gaps** are `plan/SITTING4_PREP1_EXIT.md` §3.5's three, each verified at source.

> R324 — (a) SITTING4-PREP-1 is RATIFIED: the pool verdicts (sealbot
> repaired at mechanism BUILD_ABSENT; kraken and strix remain R139 loud-skips
> as rulings, strix's blocker narrowed to its pin by ANNOTATION 10);
> strength_floor landed with four biting planted breaks; the F-816-25 field
> ruling RATIFIED with its honest census — eleven sites, not five, eta_min's
> 50× divergence included. Sitting 3's stated sealbot cause is CORRECTED on
> the record: the pin was always present; the build was absent.
> (b) The ~50 s figure is corrected at its three carry sites in place; the
> measured basis is ~341 s at the single-eval-worker rate, producer named.
> (c) The sealbot repair becomes reproducible: a pinned fetch-and-patch script
> with its sha lands in-tree, so the box runs the same commands from the tree,
> not from memory. Strix's rung MAY be pinned the same way if its adapter
> exists at contact; else its scope is reported, the loud-skip stands.
> (d) strength_floor values stand PROPOSED (probe_games 4, min_decisive_rate
> 0.25, min_winrate 0.0, the strength_floor_refused reason branch implemented)
> and NOT armed; their deciding ground — candidate-vs-random decisiveness,
> never measured in any era — is measured this mission; arming remains the
> operator's sitting-4 forwarding.
> (e) NIGHTRUN SCOPE ADOPTED, legs severable in order: hygiene; perf
> tranche-2 (profile before fix; the geometry-independent concurrency lever;
> collate_h2d's non-copy share; the fuse's source-side reads; the 7.2%
> instrument discrepancy); the floor lane (S-INCR-GRAPH under its external
> witness, then a TIME-BOXED kernel feasibility spike behind a parity oracle);
...
# R326 — architect ruling, ARCH-ERA session, 2026-08-31 (RECAL-SITTING-4 RATIFIED on a REPRODUCED card-level fact; THE CAP RULING — the trainer's sizing permission re-derived as measured need plus a stated allowance and PROPOSED at 8.40 GiB, conditional on pre-flight's independent boundary re-derivation; the `_MOVED_LEAVES` widening GRANTED; gap-4 FINAL — the corpus-mix loader DELETED with a grave line; `sealbot_d6` EXCLUDED from the default battery and `make vendor` made idempotent; the sitting-5 sequence set — inherit-or-re-measure, fit, conjunct 2, the nine-act mint, then THE OWED BURST) [INLINE]

**Provenance: [INLINE], authored.** Text originates in the ARCH-ERA architect session under R303(d), forwarded by the operator into this RECAL-SITTING-5 execution session's prompt rather than as a file — so the landing session CREATED the ruling's one canonical home, `PACKET_R326_SITTING5_LAUNCHER.md` §1. Byte-diff on append per R306(a) (de-marked, whitespace-normalized, line-by-line; extraction by `awk` over the canonical home, never retyped; extraction and diff under `set -o pipefail`, R313(c)). **The forwarded text carried the placeholder `R{next}`**, substituted to `R326` after verifying the head is R325 — census `R23–R325, 296 sections / 296 distinct / 0 duplicates, excluded set 53`, missing-in-range `{24, 29, 32, 33, 227, 228, 267}`; `--stamp` reported `STAMP OK: stamp v3.31 == §8 last entry v3.31`; `--self-test` all four controls fire; last register header `R325`, no `R326` heading present. **The LIVE-placeholder check `grep -rn '^> *R{next}' plan/` returns ZERO hits, the correct reading for a prompt-forwarded landing** — the same shape as R323's, R324's and R325's. What stands in its place: the forwarded text was transcribed a SECOND time, independently, with `R{next}` intact, and the two transcriptions were diffed BEFORE substitution — IDENTICAL, 42 lines (`wc -l` over the extracted block, derived at the check); the substitution was then applied mechanically by `sed` to both, re-diffed, and the canonical home's `awk`-extracted block diffed against it — EMPTY. **`grep -rn 'R326' plan/` did NOT return nothing at all before the canonical home was written, and the departure from the R323–R325 formula is stated rather than smoothed over: it returned exactly ONE hit, `RECAL_SITTING4_RECORD_2026-08-31.md:12`, which is sitting 4's own §1c pre-flight line recording that the same grep returned nothing FOR IT.** That is a reference to the check, not competing canonical text, and the ONE-TEXT rule is unbroken.

**The ruling's referents re-derived at point of use before landing, not transcribed from the exit screen — and (b)'s second parenthetical figure is MEASURED DIFFERENT, which is recorded here because the clause it sits in is the one carrying a HALT trigger.** **Clause (a)'s "within 17 KiB"** is `plan/RECAL_SITTING4_RECORD_2026-08-31.md` §5.3: headroom `+0.169357` this sitting against `+0.169374` at 2026-08-28, a difference of `0.000017` GiB = **17.8 KiB**; the record's §5.5 comparison table shows every OTHER partition term agreeing EXACTLY (`usable_card`, `cuda_context`, `eval_child`, the fitted pair) or to 7 s.f. (`frag`), so "every partition term within 17 KiB" is true with the headroom as its loosest member. **"93 commits" is DERIVED, not carried:** `git rev-list --count e3992c0..3e7fc54` = **93**, against sitting 3's own recorded box HEAD at `RECAL_SITTING3_RECORD_2026-08-28.md:217`. **Clause (a)'s "flat self-play peak"** is the record's §3 ladder table, `2.5618 GiB` on all six rungs from 2 workers to 16. **The "four self-caught errors"** are `plan/RECAL_SITTING4_EXIT.md` §7.1 — the eval probe's `run_gate: false`, the WITHDRAWN Δ10.2 budget-formula claim, the marker-less "full tier" run, and lever 1's asserted-then-computed threshold — and the `secret_scan` misuse is §7.2, distinct from them. **Clause (b)'s `7.443 GiB` is BYTE-IDENTICAL across the two sittings**, `peak_delta = 7 992 252 928 B` at the leg-2 oracle in both (`RECAL_SITTING3_RECORD_2026-08-28.md:354`, `RECAL_SITTING4_RECORD_2026-08-31.md:275`); the `/ 7.449` in §5.5's row is sitting 3's own prose variant at `:1090` and is not a second measurement, so "reproduced to kilobytes" UNDERSTATES the agreement rather than overstating it. **`8.40` is `7.443 × 1.1286`, i.e. `+12.86 %`, which the ruling states as `12.9 %`.** **The boundary is RE-DERIVED INDEPENDENTLY and it REPRODUCES: 8.4669 against the sitting's 8.4666, a 0.004 % disagreement** — the full expression reconstructed from the artifacts (the fit's achieved fraction `k = 391 742 772 / 460 890 719 = 0.84997` carries the coupling, because lowering the trainer's budget RAISES STEP 2's leftover and therefore the self-play term), bisected, and CONTROLLED by reproducing the sitting's own reported terms at `9.431`: trainer `11.777304` exact, `selfplay_inference` to 16 µGiB, headroom to 17 µGiB. **The clause's FIRST margin reproduces exactly — `8.40 − 7.443 = +0.957`. Its SECOND does not: conjunct-2 headroom at 8.40 is `M + 0.0125`, not `M + 0.083`.** The architect's figure is the budget slack below the boundary converted at `frag` (`0.0666 × 1.24879 = 0.0832`), which is the self-play term held FIXED — the same uncoupled reading the sitting's §7 lever-1 paragraph explicitly avoided by bisecting the full expression. **Under the full expression only `(1 − k)` of a trainer reduction accrues to headroom, so the proposal clears M by 0.0125 GiB, 3.6 % of M, not by 0.083.** THE HALT DOES NOT FIRE: (b) keys it to a *boundary* disagreement, the boundary agrees to 0.004 %, and the proposal clears on both sides — but it clears the partition side **6.6× thinner than the clause predicts**, and the sitting is entitled to know that before it fits. **A sensitivity the derivation printed and this entry carries because it is structural rather than arithmetical:** at `k = 1` (a fit that saturates its budget) headroom collapses to the constant `M − cuda_context = 0.089` at EVERY budget and no value of `_SIZING_BUDGET_GIB` closes the partition. The partition therefore closes only because the discrete cap search under-fills its budget by ~15 %, and that is the `cuda_context` double-count of lever 3 (`RECAL_SITTING4_RECORD_2026-08-31.md` §7.3) wearing different clothes. R316(b)'s rule stands untouched — correcting the double-count VOIDS M — so nothing is corrected here; it is recorded. **Clause (c)'s subject** is `tests/config/test_minted_config_remint.py:238`, `_MOVED_LEAVES = {("run5.yaml", "selfplay.n_workers")}`, whose own comment reads *"Widening it FURTHER is a ruling, not an edit"*; the file is **NOT** in any `wp/*/ORACLE_FREEZE*.sha256`, derived per path, so no frozen-file grant is owed and (c) is the ruling that comment demands. **The seven** are `_CONFIGS` (six) plus `_POST_BASELINE_MINTS`' one (`shakedown_20260807.yaml`), which is the same seven R316(f) mints the pick into. **Clause (d)'s subject** is `src/mantis/train/batch_assembly.py:131`, and the rationale it expires is that module's own docstring paragraph — *"It MUST NOT be deleted: this is the corpus-mix candidate mechanism of the BOOTSTRAP-CORPUS prereg row"* — whose stated ground is that the decision was untaken. **Posture (A) is signed and CLOSED** at `plan/RUN5_MINT_PREREG.md`'s “VALUED 2026-08-31” block (the operator's word carried by the sitting-4 launcher §0.2, committed `ea79794`), so R289(q)'s premise has moved and (B) is excluded by definition; R325(c) left the choice to a derivation and this is the derivation's ground arriving. **R289(q) is read VERBATIM before being departed from, and the departure is PARTIAL and stated as such:** its words are “the mixed-batch/pretrained-buffer PATH is RESERVED, not dead … MUST NOT be deleted”, and its stated ground is that the path is *the corpus-mix candidate mechanism of the bootstrap prereg row* — an arm of a decision then untaken. The decision is now taken. R326(d) deletes the LOADER, a strict SUBSET of that path; the mixed-batch assembly arms in the same module are NOT touched and R289(q) continues to govern them. A later ruling superseding an earlier one on an explicitly moved premise is the register working; a later ruling superseding one silently is what it exists to stop, so the supersession is named here rather than left to be inferred from a diff. **Clause (e)'s `30.9 s`** is `plan/SITTING4_PREP1_EXIT.md:93` — mean per first move at depth 6 over three book positions, range 15.331–42.176 — against `configs/run5.yaml:25` `round_timeout_sec: 3600.0` and `:65` `games_max: 32` on the `sealbot_d6` rung at `:58`. **R139's loud-skip mechanism** is `src/mantis/bots/resolve.py:44-52` (`_R139_SKIP_GROUNDS`, `_R139_SKIP_MARKER`) feeding `SKIP_REASON_MARKERS["operator_authorized"]`. **`make vendor`'s non-idempotency** is `tools/vendor_fetch.sh`'s unconditional `git apply`, recorded at `RECAL_SITTING4_EXIT.md` §7.3.

> R326 — (a) RECAL-SITTING-4 is RATIFIED: the card-level fact REPRODUCED —
> every partition term within 17 KiB across 93 commits and a changed worker
> geometry, so the refusal is a property of the card and the pre-registered
> permission, not of any measurement. Phase W's pick 14 stands by the
> pre-registered rule; R317(d)'s carried-noise assumption is MEASURED FALSE
> and corrected of record — future ladders use per-rung noise, and the error
> ran in its own conservative direction. The flat self-play peak (2.5618 GiB,
> 2→16 workers) is recorded: the inference share does not scale with the
> pick. The probe (0.314% agreement), the round series (PLATEAU with its band
> sensitivity printed), and the rung control are ratified. The session's four
> self-caught errors and the secret_scan misuse stand on its ledger; the
> launcher's missing ruling text was the ARCHITECT'S omission, and refusing
> to invent one was correct.
> (b) THE CAP RULING. The trainer's sizing permission was never derived; it
> is RE-DERIVED as measured need plus a stated allowance: peak 7.443 GiB,
> reproduced to kilobytes across two sittings, plus 12.9% → PROPOSED
> **8.40 GiB**. The ceiling convention is UNCHANGED — the term remains the
> permission; the permission shrinks to honesty. Pre-flight independently
> re-derives the closing boundary (the sitting reported 8.4666); the proposal
> must clear it with margin on both sides (trainer +0.957 over measured;
> conjunct-2 headroom ≈ M + 0.083). A boundary disagreement HALTs before ssh.
> The armed value is the operator's, enacted by this forwarding.
> (c) _MOVED_LEAVES WIDENING GRANTED: the frozen allowance extends from
> run5-only to the seven R316(f) configs; one row, flip-one-byte standard,
> planted break red before the edit and green after, neighbours untouched.
> (d) GAP-4 FINAL: with posture (A) signed, the loader's (B)-serving
> rationale is EXPIRED — load_pretrained_buffer is DELETED with a grave line;
> the import-census oracle pins the absence.
> (e) sealbot_d6 × 32 is EXCLUDED from the default battery by ruling — it
> cannot finish inside its own timeout at the measured 30.9 s/move; loud-skip
> per R139, revisited at the gate-geometry re-adjudication. make vendor is
> made idempotent as ordered hygiene.
> (f) SITTING-5 SEQUENCE: terms and the pick INHERIT from sitting 4 IFF the
> box sha is unchanged at 3e7fc54 apart from this packet's own commits — any
> other drift re-measures, no exceptions. Fit with the armed cap; conjunct 2
> must clear M under the ceiling convention; the nine-act mint includes the
> cap row, the pick, the posture row already valued, the allocator posture,
> and the strength_floor values; then THE OWED BURST runs at the fitted caps
...
# R332 — architect ruling, ARCH-ERA session, 2026-09-03 (REPAIR-1 RATIFIED — P1–P4 with pinning tests in the same commits, the local runner making the gate set ONE command, the three self-introduced reds ledgered with their one cause, and the F-14/F-11 banks accepted; the R118/A-1 FREEZE on `eval/rounds.py` LIFTED with the standing RULE that a freeze outlives its subject only by ruling; F-11's arming mechanism declared MINT-CLASS and screened, not armed; REPAIR-2 ORDERED as three legs P5 → P8 (F-16 first, F-19's warm-start entry BUILT) → P6 with the EMA key landing as a schema twin default-off; REPAIR-3 and the order after it SET; run6 REMAINS HELD) [INLINE]

**Provenance: [INLINE], authored.** Text originates in the ARCH-ERA architect session under R303(d), forwarded by the operator into this REPAIR-2 execution session's prompt rather than as a file — so the landing session CREATED the ruling's one canonical home, `PACKET_R332_REPAIR_2.md` §1, and no competing text existed under `plan/` before it: `grep -rn 'R332' plan/` returned **rc 1 and 0 hits**. **The forwarded text carried the placeholder `R{next}` in TWO places** — the dispatch's §1 heading and the quote-block lead — both substituted to `R332` after verifying the head is R331: census `R23–R331, 302 sections / 302 distinct / 0 duplicates, excluded set 53`, missing-in-range `{24, 29, 32, 33, 227, 228, 267}`; `--stamp` reported `STAMP OK: stamp v3.37 == §8 last entry v3.37`; `--self-test` all four controls fire. The residual-placeholder grep over the canonical home's §1 returns **rc 1**, and §1 carries exactly ONE `R332` token — the quote-block lead — so no clause number was rewritten. Extraction into the register is by heredoc over the canonical home's §1 block, never retyped from the prompt a second time.

**The ruling's referents re-derived at point of use before landing.** **Clause (a)'s "pinning tests in the same commits"** is DERIVED, not accepted: over `ba883c9..HEAD` every commit whose subject is a repair carries at least one file under `tests/` or `crates/*/tests/` in that same commit (counts 1–26); the five commits carrying none are the governance-mirror regenerations, the doc-only Leg 4 commit, the floor ratchet and the `.pyi` stub fix — none of them a repair. The tip is **28** commits past `ba883c9`, not the 27 `REPAIR1_EXIT.md` §0 states: the 28th (`4f5a114`) is the mirror regeneration that followed the exit screen's own writing, so the exit file is right about the tree it described and stale about the tree today. **"Nineteen gates"** is measured, and the measurement corrects a first reading of my own: `grep -c 'run_gate ' run_all.sh` = **20**, and the definition line is `run_gate() {` with no space, so it is NOT among them — all twenty are invocations. Gate 1 is the one guarded by `[ $WITH_FRESH_SYNC -eq 1 ] &&`, leaving **19 that run by default**, which is the clause's number and also the `green: 19 / ALL GREEN` line of the run of record. The summary block prints gate 1's omission unconditionally, so "opt-in and self-declaring" is structural, not a claim about one run. The floor file reads **4629**. **Clause (b)'s freeze is measured, and its shape is not what the phrase "freeze manifest" suggests:** `grep -rn 'eval/rounds.py' wp/*/ORACLE_FREEZE*.sha256` returns **rc 1, zero rows** — the producer was never in a sha256 freeze register at all. The freeze existed as exactly two things: PREREG_A §8 abort 8, and the in-repo working-tree guard `tests/eval/test_wr_sealbot_config_only.py::test_the_wr_sealbot_producer_file_is_unmodified`, which shells `git diff --stat` over `src/mantis/eval/rounds.py` and fires on ANY uncommitted edit. The ONE freeze row that traces to the file is the GUARD's own file, `ORACLE_FREEZE_A.sha256:141` at `d4c3337…`, green at landing — so "rows tracing to this file close with it" is one row, and closing it moves one sha under this ruling's authority. **Clause (b)'s "its subject merged"** is R118's own text at `:1525` — *"A-1 (wr_sealbot None until WP12-R): ratified; value populates in WP12-R Phase A"* — and Phase A is long merged (Phase T closed at R162, 2026-08-01); the value populating is itself pinned on dev by that same file's `test_a_resolvable_sealbot_rung_makes_wr_sealbot_a_float_with_no_producer_edit`. A guard whose ruling's premise has been discharged for a month was still refusing edits to a live producer, which is the cost clause (b) names. **Clause (d) carries ONE mis-citation, corrected of record here rather than repaired silently:** there is no EMA key in F-08. F-08 is *"`python -m mantis.run` installs no logging handler"* (class 1, R1); the EMA arming key is **F-06** *"EMA is a lever with no arming key"* (class 3, R1), which the dispatch §3 already lists in P6's roster. The clause is read as F-06 and the decided shape — schema twin, default off, value a prereg lever — is F-06's first repair option. Separately and because the id was named: **F-08 is the only R1 finding the audit's §5 order assigns to NO packet** (P9's rows are F-47, F-48, F-33, F-19), and it is the row that makes F-11's newly-landed `disk_guard_error` warning invisible — so it is repaired in Leg 3 beside F-06, with the correction stated. **Clause (c)'s premise** is verified at HEAD: `armed_aborts` `DISK_SPACE_ABORT_RULE` is `CONFIG_THRESHOLD_GT_ZERO` with `cadence_paths=()`, so gate 12 still audits the REQUIRED `disk_space_exhausted` row ARMED off `monitor.disk_guard.fail_gb > 0` — a config number, not a producer — exactly as `REPAIR1_EXIT.md` §2 banked it.

> R332 — (a) REPAIR-1 is RATIFIED: P1–P4 repaired with pinning tests in
> the same commits; the local runner makes the gate set one command (nineteen
> gates; gate 1 opt-in and self-declaring); the three self-introduced reds,
> each caught by a gate, stand on the ledger with their cause named once;
> the F-14 and F-11 banks are accepted.
> (b) The R118/A-1 FREEZE on eval/rounds.py is LIFTED: its subject merged and
> its certification is discharged; the freeze manifest and freeze_verify
> reconcile (rows tracing to this file close with it); the pipeline-published
> identity with its agreement check is the pinning test; F-14's producer half
> completes under normal discipline. RULE: a freeze outlives its subject only
> by ruling.
> (c) F-11's arming mechanism is MINT-CLASS: presented at this session's
> pre-flight in one screen, two shapes costed; the architect rules at exit;
> nothing arms here.
> (d) REPAIR-2 executes P5, then P8 with F-16 first and the F-19 warm-start
> entry BUILT (identity names a checkpoint by path and net hash; the one
> selector authority resolves it through the artifact stamp; pinning test:
> the loaded net's hash equals the checkpoint's; a CPU smoke plays one legal
> game from it) — the 20/20 step-0 reproduction runs at the mint sitting on
> the checkpoint of record, not here; then P6, with F-08's EMA key landing as
> a schema twin, default off, its value a prereg lever.
> (e) REPAIR-3 (P7, P9) follows; the dashboard and the tranche-2 prereg after;
> the run6 mint after those. Run6 remains held.
> ROUTE: land, census, ACTIVE next; pre-flight screen; Legs 1–3; one exit.

# R333 — architect ruling, ARCH-ERA session, 2026-09-03 (REPAIR-2 RATIFIED — P5/P8/P6 executed, the warm-start entry built with its hash-equality pin, F-16 closed so V2 survives an eval round, the two wrong audit repair lines corrected of record and the four banks accepted; **THE TIER LAW** — a tier-scoped verification loop cannot see a tier-scoped call site, so a stricter signature sweeps the WHOLE TREE by structure and the slow tier joins the one command as a self-declaring opt-in; F-11's and F-32's arming RULED ON READING with the screen riding to exit verbatim and F-18's bank ground restated at pre-flight; REPAIR-3 ORDERED as four legs — P7, P9 with F-19's dead-code half measured against the entry that now exists, the runner change, then DASH-1 the file-based run dashboard; the order after it SET; run6 REMAINS HELD) [INLINE]

**Provenance: [INLINE], authored.** Text originates in the ARCH-ERA architect session under R303(d), forwarded by the operator into this REPAIR-3 execution session's prompt rather than as a file — so the landing session CREATED the ruling's one canonical home, `PACKET_R333_REPAIR_3.md` §1, and no competing text existed under `plan/` before it: `grep -rn 'R333' plan/` returned **rc 1 and 0 hits**. **The forwarded text carried the placeholder `R{next}` in TWO places** — the dispatch's §1 heading and the quote-block lead — both substituted to `R333` after verifying the head is R332: census `R23–R332, 303 sections / 303 distinct / 0 duplicates, excluded set 53`, missing-in-range `{24, 29, 32, 33, 227, 228, 267}`; `--stamp` reported `STAMP OK: stamp v3.38 == §8 last entry v3.38`; `--self-test` all four controls fire. The residual-placeholder grep over the canonical home's §1 returns **rc 1**, and §1 carries exactly ONE `R333` token — the quote-block lead — so no clause number was rewritten. Extraction into the register is by script over the canonical home's §1 quote block, never retyped from the prompt a second time.

**The ruling's referents re-derived at point of use before landing.** **Clause (a)'s "P5, P8, P6 executed"** is measured, not accepted: `git log --oneline 4f5a114..eaeb1d2 | wc -l` = **10** commits, the collected-test floor file reads **4730** (from 4629), and `REPAIR2_EXIT.md` §5's gate table is ALL GREEN 19/19 on that exact tree. **"the warm-start entry built with its hash-equality pin"** — `identity.warm_start` is a real optional BLOCK at `src/mantis/config/schema/core.py:160` whose docstring names its consumer chain `mantis.train.warmstart.resolve_bc_warm_start` → `apply_bc_warm_start`; the entry is reached from production at `src/mantis/train/orchestrator.py:148-150`. **"F-16 closed"** — `src/mantis/eval/snapshot.py:29` now reads `_ARCH_TYPES: dict[str, type] = ARCH_KINDS`, the hand copy replaced by the authority itself. **"the four banks"** is four ROWS but not four of one kind, and the distinction is worth keeping: `REPAIR2_EXIT.md` §4 carries THREE banked (F-18's r8 parity-manifest row, F-42's one-owner axis table, F-32's arming) and ONE **REGISTERED** — F-39's 34 bridge-signature defaults, which ship with an enumerated `REGISTERED_DEBT` frozenset at `tests/config/test_no_bridge_default_shadows_a_config_key.py:54` that reds on a new offender AND on a listed row that no longer exists. A registered debt with a two-way ratchet is not a bank; accepting it as one would lose the ratchet. **Clause (b)'s slow tier is measured before the runner is changed:** `pytest -m slow --collect-only -q` reports **7 tests / 4723 deselected**, in seven files — six under `tests/model/conformance/` and one tier witness, `tests/test_bare_pytest_tier.py::test_slow_witness_is_outside_the_default_tier`. Two further files match a `mark.slow` grep and carry NO slow test (`tests/monitor/test_manifest_contract.py`, `tests/tools/test_tier_census.py`): both mention the marker as the subject of a census, which is exactly the text-not-structure trap REPAIR-2 §6 item 2 recorded — the file list is taken from the collector, never from the grep. **Clause (d)'s "F-19's decision half resolves — the entry exists"** is verified at HEAD as above; what remains for the dead-code half is the module's OTHER surface, which the audit's F-19 row did not enumerate because it was measuring the whole module as dead. **One census in the ruling's own source is corrected of record before it is inherited:** AUDIT-1 F-44 states **four** hand-mirrored schema-leaf walkers; there are **FIVE**. The audit's census was scoped to the NAME `_leaf_paths`, and the fifth walker is called `live_leaf_paths` (`tests/model/conformance/test_config_partition_shared_vs_arch_scoped.py:190`), consumed by its own module and imported by `tests/model/conformance/test_arch_selector_makes_v2_selectable.py:370`. Its docstring says outright that it performs *"the same walk `tools/ci_gates/contract_doc_gate.py` performs"* — a fifth copy that a name-scoped census cannot see. Leg 1's reconciliation absorbs it; the correction is recorded here so the number five is not read as drift from the audit.

> R333 — (a) REPAIR-2 is RATIFIED: P5, P8, P6 executed; the warm-start
> entry built with its hash-equality pin; F-16 closed so V2 survives an eval
> round; the two audit repair lines wrong at contact (F-30's graph half,
> F-35's census root) corrected of record rather than followed; the four
> banks accepted on their recorded grounds. The exit's red-then-green is the
> leg's finding.
> (b) LAW: a tier-scoped verification loop cannot see a tier-scoped call
> site. A repair that makes a signature stricter sweeps call sites over the
> WHOLE TREE by structure before it is called done; the slow tier runs at
> every packet exit (opt-in flag on the runner, its cost stated on every run,
> like gate 1). REPAIR-3 lands the runner change.
> (c) F-11 and F-32 arming: the architect rules ON READING — the F-11 screen
> rides to this session's exit verbatim; whichever shape is ruled, the arm
> lands at the mint forwarding. F-18's bank ground is restated in one line at
> pre-flight; if it is "needs a ruling," the architect rules with it.
> (d) REPAIR-3 executes P7, then P9 (F-19's decision half resolves — the entry
> exists; the dead-code half is measured against it), then the slow-tier
> runner change, then DASH-1: the file-based run dashboard.
> (e) After this: the tranche-2 prereg and the lane-C codec design (the
> architect's), the perf tranche, then the run6 mint. Run6 remains held.
> ROUTE: land, census, ACTIVE next; pre-flight lines; Legs 1–4; one exit.

# R334 — architect ruling, ARCH-ERA session, 2026-09-04 (REPAIR-3 RATIFIED and the held push EXECUTED, so `dev` and `origin/dev` reconcile before any measurement; F-11 ARMED as SHAPE A — a producer-liveness operand on the arming predicate with gate 12 untouched, plus `poll_once` reading the guard's own counters as a last-emit age, and shape B REJECTED in its form because a monitor thread on a relaunch-class stall code is a crash loop into a filling volume; F-32 RULED SHAPE A at the run6 mint with the pin DERIVED from `identity.warm_start` rather than hand-synced, shapes B and C both REJECTED with grounds, and the guard's unreachability DISCLOSED by a test until minted; F-18 RULED option (i) — the cross-crate pin is the r8 authority and the manifest's Python-oracle expectation retires with a grave line; PERF-TRANCHE-2 PRE-REGISTERED as six items with bars written into the record BEFORE each A/B, the kernel spike and Gumbel explicitly OUT; and the RUN6 SUCCESS WITNESS pre-registered IN SHAPE with its two judgment-valued terms left as operator prereg rows) [INLINE]

**Provenance: [INLINE], authored.** Text originates in the ARCH-ERA architect session under R303(d), forwarded by the operator into this PERF-TRANCHE-2 execution session's prompt rather than as a file — so the landing session CREATED the ruling's one canonical home, `PACKET_R334_PERF_TRANCHE_2.md` §1, and no competing text existed under `plan/` before it: `grep -rn 'R334' plan/` returned **rc 1 and 0 hits**. **The forwarded text carried the placeholder `R{next}` in TWO places** — the dispatch's §1 heading and the quote-block lead — both substituted to `R334` after verifying the head is R333: census `R23–R333, 304 sections / 304 distinct / 0 duplicates, excluded set 53`, missing-in-range `{24, 29, 32, 33, 227, 228, 267}`; `--stamp` reported `STAMP OK: stamp v3.39 == §8 last entry v3.39`; `--self-test` all four controls fire. The residual-placeholder grep over the canonical home's §1 returns **rc 1**, and §1 carries exactly ONE `R334` token — the quote-block lead — so no clause number was rewritten. Extraction into the register is by script over the canonical home's §1 quote block, never retyped from the prompt a second time.

**The ruling's referents re-derived at point of use before landing.** **Clause (a)'s four ratification terms are each MEASURED, not accepted.** *"P7's one walker for seven hand copies"* — a STRUCTURAL census (an AST walk for any function that both recurses on itself and reads `model_fields`) over the whole tree finds **THREE** such functions, and only ONE is a leaf-path walker: `src/mantis/config/schema/leaves.py:65::leaf_paths`, the authority. The other two are declared and are not copies — `tests/config/test_schema.py:312::walk` censuses reachable MODELS (not leaf paths) and asks `leaf_paths`' own predicate `nested_block` rather than holding a second implementation of it, and `tests/config/test_one_schema_leaf_walker.py:119::naive` is the PLANTED BREAK inside the pinning test, which must disagree with the authority or the guarantee is gone. *"P9's two reverted deletions on ruling grounds"* — `REPAIR3_EXIT.md` §3 items 1 and 2: `BatchBuffers` is RESERVED-NOT-DEAD under R289(q), and `buffer_persist.py` SURVIVES under `repo_design`'s v5→v6 amendment item 3 on R178(c); both were deleted and reverted, and in the second case the durable half — the pin — landed alone. *"DASH-1 with seven live panels and two banked"* is derived from the source rather than the exit screen: an AST read of `tools/run_dashboard.py` gives `PANELS` **9** and `BANKED_PANELS` **2** (`average sims/move`, `held-out loss`), i.e. **seven live**. *"Merge and push executed"* is the one term of clause (a) that was NOT true at landing — `dev` was **`639c75f`**, **14** commits past REPAIR-2's close `eaeb1d2`, with `origin/dev` still at **`ba883c9`**; REPAIR-3 held the push deliberately (its §7 states the mechanical reason: gate 3c's ref floor must sit behind while a leg declares a down-ratchet) and handed it to the operator. **This ruling is that forwarding, and the push is this session's pre-flight act, not a description of a completed one.** **Clause (b)'s two shapes are read at their source and their premises re-verified at HEAD**, `plan/REPAIR3_ARMING_SCREENS.md` PART 1: the `disk_space_exhausted` row is still `Mechanism.CONFIG_THRESHOLD_GT_ZERO` over `monitor.disk_guard.fail_gb` with `cadence_paths=()`, so gate 12 audits it ARMED off a CONFIG NUMBER and no runtime fact can reach a pure-config audit; the counters shape A's probe would read (`checks_total`, `errors_total`) and the named `disk_guard_error` event already exist, landed by REPAIR-1. Shape B's rejection ground is quoted from that screen and is a MECHANISM, not a preference: the stall watchdog's code is **42, the TRANSIENT class the supervisor RELAUNCHES on**, so a disk guard raising every tick would stall-abort and be relaunched into the same broken state — on the leg whose whole purpose is stopping a run before the volume fills. The screen's own cheaper follow-on for the mid-run gap — *"the guard's own counters … read by the existing `poll_once` as a last-emit age"* — is exactly what clause (b) adopts beside shape A, so the clause takes the recommendation AND the residual it disclosed. **Clause (c) departs from the screen in one respect and it is the load-bearing one.** PART 2 recommends shape A as a schema key `identity.expected_anchor_sha256`, a *"`str | None` twin of the `identity.warm_start` block"*; the ruling adopts shape A but **refuses the twin** — the pin DERIVES from `identity.warm_start` when present, one source, and a standalone key exists only for the non-warm-start case. That is a correction to the screen, not an endorsement of it: R332(d) minted `identity.warm_start` as *a checkpoint named by path AND by the hash it must turn out to be*, so a second hand-maintained hash field would be two authorities for one fact — the class R104's agreement-or-raise and R332(b)'s one-denomination repair both exist to prevent. Shape C's rejection ground is verified to exist rather than assumed: the frozen-incumbent post-mortem is `inputs/run3_findings_v2.md` A1 (`actor_sync ⊥ deploy_gate`), whose consequence is recorded at `plan/STATE_2026-07-24.md:46` as *"run3 during the frozen-actor era"* and whose repair is ledgered at `plan/port_ledger.md:42` — the actor never synced at all, **0 %, not run3's ~39 %**, and nothing was ever deploy-blessed. A guard against launching on an unverified incumbent is the instrument that class of defect argues for, which is why the refusal is WANTED. **Clause (d)'s option (i) is read at PART 3 and its negative half is verified:** `grep -rln 'wpa_positions|graph_parity'` over `*.py` returns **zero files**, so *"regenerated through the Python oracle"* has no subject in this tree, and the only thing that could emit an r8 row is the Rust builder the fixture exists to check. **Clause (e)'s six items each carry a measured antecedent, and three of the six are RE-ATTEMPTS of items a prior tranche left undone rather than new proposals** — `NIGHTRUN1_TRANCHE2_RESULTS.md` §1 records item 3 (`collate_h2d`) **NOT REACHED**, item 4 (the fuse's source-side reads) **CONDITION NOT MET** because step 1 profiled the EVAL path where the fuse does not appear, and item 2 (eval concurrency) **DEFERRED with its reason measured**. The antecedents: `collate_h2d` at **0.405 ms/sim** with **~81 %** of it not the copy (`PERF_TRANCHE1_RESULTS.md` §2.2 — the pure H2D of the measured part is 1.29 ms of a 6.74 ms stage, and the box's pageable H2D measures **14–15 GB/s**, not the ledger's DERIVED 3.11); INCR-GRAPH's margin **141×** on the self-play build and **58×** post-E1 on the eval build (`NIGHTRUN1_EVIDENCE/leg2_incr_graph_inequality.md` §3, against a parent-copy FLOOR of 0.034 ms/leaf that is a WARM figure — the file's own pessimistic re-reading puts it at ~0.19 ms and ~30×, and states that even that clears); the eval loop's **95.3 %** inside `submit_graphs_and_wait_ls` (`leg1_eval64_profile.md` §1); and the fuse's **16.8 ms/pop** invariant under A1's ×6.79 rewrite (`PERF_TRANCHE1_RESULTS.md` §2.1). The audit's F-51 supplies each item's candidate MECHANISM — HOT-04 for T2-1 (the serve-path collate reads the edge arrays ~6× on the host against 1× for the copy; check 7 is implied by check 8, `_graph_of` computed twice, `np.unique` sorts), HOT-11 for T2-3 (one serial game stream, exactly one 8-leaf batch in flight, fill 8/64, card 85–87 % idle), HOT-14 for T2-4 (the invariant is cold reads of graphs twelve idle workers built). **Clause (e)'s "the 7.2 % discrepancy" is NOT an open question in the form the dispatch states it:** NIGHTRUN-1 §4 already BOUNDED it as within this box's own measured run-to-run spread (**CV 5.61 %, min-to-max 15.55 %, n = 6**) and recorded that the original pair is **unresolvable by construction**, both readings having been taken on an instance since recreated. T2-5 therefore inherits a closed bound and can only widen or re-scope it — and it should be re-scoped, because that spread was measured on the EVAL path and the 7.2 % was observed on the SELF-PLAY contended card. **Clause (f)'s `sealbot_d5` is a real rung**, `configs/run5.yaml:48-49`, and the two terms the clause leaves to the operator (N, the slope bar) are the two that carry judgment; the shape does not.

> R334 — (a) REPAIR-3 is RATIFIED (P7's one walker for seven hand copies;
> P9's two reverted deletions on ruling grounds; the slow tier at packet
> exit; DASH-1 with seven live panels and two banked). Merge and push
> executed; pre-flight verifies dev == origin/dev.
> (b) F-11: SHAPE A adopted — the arming predicate gains a producer-liveness
> operand, gate 12 untouched — plus poll_once reading the guard's counters as
> a last-emit age for mid-run death. Shape B is REJECTED in its form: a
> monitor thread on a relaunch-class stall code is a crash loop into a
> filling volume. Lands BEFORE the mint.
> (c) F-32: SHAPE A at the run6 mint. The pin DERIVES from identity.warm_start
> when present — one source, no hand-synced twin; a standalone key exists
> only for non-warm-start launches, decided by derivation at mint prep. Shape
> C is REJECTED (the refusal is wanted — run3's frozen-incumbent post-mortem
> is its ground); Shape B is REJECTED (an invocation is not an identity).
> Until minted, the guard's unreachability is DISCLOSED by a test, not
> hidden.
> (d) F-18: option (i) — the cross-crate pin against mantis-core's
> independent implementation is the r8 authority; the manifest's
> Python-oracle expectation retires with a grave line; (iii) rides lane C's
> golden recapture; (ii) is rejected.
> (e) PERF-TRANCHE-2 PREREG: the six items below, bars from PERF_RESEARCH's
> conservative bounds written into the record BEFORE each A/B; rig-based,
> regime named per figure; net-hash green after every landed change; memory
> deltas recorded for the mint that follows; the frontier ledger
> re-baselined at exit at the r8 identity. The kernel spike and Gumbel are
> OUT — one is deferred, the other is the operator's prereg row.
> (f) The RUN6 SUCCESS WITNESS is pre-registered in shape: (i) the promotion
> gate completes rounds from round one; (ii) a checkpoint beats sealbot_d5 at
> fixed nodes with a CI excluding 0.5 within N games; (iii) Elo against the
...
# R336 — architect ruling, ARCH-ERA session, 2026-09-04 (PERF-TRANCHE-3 RATIFIED and the SITTING-7 PRE-FLIGHT HALT RATIFIED AS A SUCCESS with all four blockers placed on the ARCHITECT'S ledger — a registry row named as an arch kind, a circular mint act, a forwarding precondition the packet could not carry, two row-number collisions; THE SELECTOR ROW RULED IN BOTH HALVES — `identity.encoding = gnn_axis_r8` and the arch kind `GnnArchV2` on W-C1's measured 78.5× vs 2.49× hazard, with the row's SHAPE one validated key read by the one selector authority and config-less sites resolving from the STAMP, a site holding neither being a FINDING and not a third path; THE MINT ACT SEQUENCED AGAINST ITSELF — the caps' act at STEP 3 and `identity.warm_start` as a SEPARATE PIN ACT at 4a/4b/4c, because a hash cannot be minted before the artifact exists and v1 asked for exactly that; SIX PREREG ROWS RE-SHAPED TO HEAD by the filing report's own findings with TWO DROPPED — the NaN row because arming an envelope against a mechanism that does not exist is the §3 error repeated, the collapse-abort row because G-3 stands and the external instrument is READ, not armed; and rustfmt RULED ON CONTACT — the line becomes "on touched files", `rustfmt.toml` is committed so the style has a definition, and no tree-wide pass follows) [INLINE]

**Provenance: [INLINE], authored.** Text originates in the ARCH-ERA architect session under R303(d), forwarded by the operator into this RUN6-MINT-LAUNCHER-v2 session's prompt rather than as a file — so the landing session CREATED the ruling's one canonical home, `PACKET_R336_RUN6_MINT_LAUNCHER.md` §1. The pre-existence grep over `plan/` returned **exactly one hit** and it is not a competing text: `RUN6_MINT_SITTING7_PREFLIGHT_HALT.md:140` reads *"There is no R336"* — a negative statement about the register's head at the moment the v1 sitting halted, which this landing makes false rather than contradicts. **The forwarded text carried the ruling-number placeholder in five positions** — the dispatch heading, three §0 clause citations, and the quote-block lead — of which **exactly one is a substitution site inside the ruling text**, the lead; no clause letter was rewritten, and the canonical home's residual-placeholder grep returns **rc 1**, the placeholder's literal spelling appearing nowhere in the file including its own landing record. Head verified by script before substituting, never remembered: census `R23–R335, 306 sections / 306 distinct / 0 duplicates, excluded set 53`, missing-in-range `{24, 29, 32, 33, 227, 228, 267}`; `--stamp` reported `STAMP OK: stamp v3.41 == §8 last entry v3.41`; `--self-test` all four controls fire; R335 is the register's LAST section with no foot annotation after it, its final line being its own `ROUTE:` line. Extraction into this register is by script over that §1 quote block, never retyped.

**The ruling's referents re-derived at point of use before landing.** **Clause (b)'s SHAPE half was verified BUILT rather than ordered, and this corrects the v1 halt's live reading of it.** `RUN6_MINT_SITTING7_PREFLIGHT_HALT.md` §2.1 carried forward `RECAL_SITTING6_RECORD_2026-09-02.md` §10's open design question — three call sites holding no validated config, and a fork between growing a second arm and refusing a run5 checkpoint at load — and marked the shape UNCHOSEN. At HEAD it is neither fork: `stamped_arch_kind` (`src/mantis/train/checkpoints.py:214`) is *"THE ONE answer to 'which arch kind does this ARTIFACT carry' (R330(e))"* in the tree's own words, *"Never reads a config; a config-less call site that needs an artifact's arch comes here and nowhere else"*, and **all three sites the halt named now route through it** — `checkpoints.py:701`, `checkpoints.py:803`, `pretrain/validate.py:56`. The halt's §2.1 was reading a 2026-09-02 record rather than HEAD, and its VALUE half was the only live one. **Clause (b)'s key and value both resolve at HEAD:** `ARCH_KIND_ROW = "identity.arch_kind"` (`src/mantis/model/arch.py:166`), an OPTIONAL leaf whose schema docstring already states the mint discipline (*"it enters production configs ONLY as a minted row at run6's mint"*, `src/mantis/config/schema/core.py:137-147`) and that *"Artifacts never read this row — a checkpoint's arch is its stamp's"*; `GnnArchV2` is a real member (`arch.py:80, 131`) and `graph` admits exactly `("GnnArch", "GnnArchV2")` (`arch.py:139`), which is what made the value a choice and not a derivation. `configs/run5.yaml`'s identity block carries `encoding: gnn_axis_v1`, `representation: graph` and **no** `arch_kind`, so both identity rows are deltas. **Clause (d)'s three at-HEAD assertions verify:** eval concurrency is a keyword-only parameter with no key — `src/mantis/arena/match.py:201` `concurrency: int = 1`, *"THE DEFAULT IS UNCHANGED AND UNARMED"* (`:219`), and `games_in_flight` is absent from the whole tree case-insensitively (`RUN6_PREREG_PROPOSAL.md` §3.1), so `G=1` is today's serial path byte-for-byte and needs nothing written; the ply cap is already ONE key, eval's `spec.max_plies` BEING `selfplay.max_game_moves` resolved once in the parent (§3.2, F-816-11), so a second number would have nowhere to go; and the NaN row's mechanism is absent by a RATIFIED decision, `src/mantis/train/coordinator/step.py:537-550` naming ADJ-D13 and the R56 SOURCE PIN in its own comment and `math.isfinite(step_gn) and …` letting a NaN RESET the counter it would be compared against. **Clause (e)'s two subjects verify:** `rustfmt.toml` does not exist at `57a59fb`, and `CLAUDE.md:113` is the line the clause moves; the Gumbel undershoot pin is real and one short — `crates/mantis-selfplay/tests/served_sims_exact.rs:260` asserts 599 served at 600 configured for `gnn_axis_r8`, with `:224` recording it as *"a tripwire, so whoever arms Gumbel sees this line first"*. **Δ18(4c)'s rung set verifies:** `vendor/pins.toml` carries `[pins.sealbot]` and no other pin, so *"random + `sealbot_d5` (the only pin at HEAD)"* is a derivation and not a preference. **ONE REFERENT DOES NOT VERIFY, and it is recorded rather than repaired silently.** Clause (d) and Δ18(4a) both cite *"BC-EXEC-1's pinned recipe"* for *"the five heldout flags as pinned"*. BC-EXEC-1's recipe is `MINT_CLOSE_BC_EXEC_1_EXIT.md` §3.2, and it pins epochs 5, peak lr 0.001, batch 256, `augment: False` and a 10 690-step budget while recording **held-out split and early stop as `NO MECHANISM`, the envelope explicitly NOT closing on them** — those two terms had no machinery when it was written, which is the R327 blocker R328(d) was ordered to fix. The five flags are pinned in **`BC_EXEC_2_PREREG.md`** (RUN6-IDENTITY-1 Leg 2): split seed 328, `heldout_frac` 0.05, game-level, `eval_every` 500, `patience` 3, and `min_delta` **measured at pre-flight** under a refusal that raises `GraphPretrainError` if it sits inside the sampler's own noise. The referent is unambiguous — BC-EXEC-2 is BC-EXEC-1's successor for the r8 identity and the only document that pins the five — so this is a citation off by one document, not a missing recipe; it joins the architect's ledger beside clause (a)'s four and the execution reads BC-EXEC-2.

> R336 — (a) PERF-TRANCHE-3 is RATIFIED: served sims now equal
> n_simulations on the PUCT path (three loops in three states, mechanism
> to the unit); S-CHECK17 landed with its bar MISSED and BANKED at ×1.18
> derived, the decomposition having falsified the bar's premise (check 14
> is compiled and is not free); S-PREFUSE REFUTED on a dev-host measurement
> with HOT-14 RE-OPENED (cross-core ownership explains ×1.66 of ×6.84);
> S-BATTERY-G landed unarmed, CUDA arm owed at this sitting's battery. The
> SITTING-7 HALT is RATIFIED as a success — all four blockers were in the
> architect's texts and are on the architect's ledger: a registry row
> named as an arch kind (inherited from the handoff and repeated), a
> circular mint act, a forwarding precondition the packet could not carry,
> and two row-number collisions.
> (b) THE SELECTOR ROW: identity.encoding = gnn_axis_r8; the arch kind =
> GnnArchV2. Grounds: V2 is the seam's proving tenant with deliberate
> detectors (R309(d)) and W-C1 measured V1's hazard at 78.5× against V2's
> 2.49× under 64× nodes. The row's SHAPE: one validated config key on
> RunConfig, consumed only by the one selector authority; config-less
> sites resolve from the stamp (R330(e)) — a site holding neither is a
> finding, not a third path.
> (c) THE MINT ACT is the caps' act (R281(d)(i) governs the PARTITION, not
> every row): STEP 3 mints both caps, the allowance row, the identity
> rows, and every ARMED prereg row. identity.warm_start is a SEPARATE PIN
> ACT: STEP 4a re-produces the checkpoint of record on the minted world
> (R329(b)), STEP 4b writes path + sha256 + net hash as its own commit
> with the F-32 pin derived from it, STEP 4c runs the step-0 witness. A
> hash cannot be minted before the artifact exists; v1 asked for that.
> (d) Six prereg rows are RE-SHAPED to HEAD, by the filing report's own
> findings: the ply cap is ONE key; the BC checkpoint of record is a
> REPRODUCTION under BC-EXEC-1's pinned recipe, so no epoch row applies to
> it; the step-0 anchor IS the warm_start artifact, not a periodic
> checkpoint; the NaN row is DROPPED (no mechanism exists — ADJ-D13's
> known gap under an R56 pin — and an envelope armed against a missing
> mechanism is the §3 error, again); the collapse-abort row is DROPPED
> (G-3 stands; the external instrument is read on the dashboard, not
> armed); N per rung stays at the minted gate geometry (R319/R320 are not
> re-opened) and witness (ii) is POOLED over a window of four consecutive
> rounds. eval concurrency has no config key at HEAD; run6 runs the
> battery serial (G=1); the key is ablation-era work.
> (e) rustfmt: CLAUDE.md's line becomes "rustfmt on touched files";
> rustfmt.toml with defaults is committed so the style has a definition;
> no tree-wide format pass (standards on contact). check 14's 41.4 ms/part
> is CARDED, not ordered — the trainer is not the run's bottleneck.
> Gumbel's one-short undershoot is pinned and is a PRECONDITION of arming
> the PCR/Gumbel pair.
> (f) The mint executes as v1 Δ12–Δ19 with the amendments in §2. Run6
