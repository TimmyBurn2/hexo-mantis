...
DISCHARGED by item (18). **Exit:** `make gates.exit` on the leg's tip (`23003bb9`) ALL GREEN 2026-09-21 12:00 UTC (1 h 42 min: default tier
...
(18) R366 (2026-09-21, dev only; 0 box-h — the box is destroyed and the next one is fresh): the register at R366
(next R367), R365's Status line amended (the < 0.25 band's premise did not hold: the decomposer found no tail-only
row on any run8 ring). **THE TREE, six commits, one per item:** (1a) contract v35 — the `model` section with
`model.gnn: {hidden, num_layers}` as graph-scoped leaves (`select_arch` read two FLAT keys no schema carried, so
every net on record was 4×128 and the conformance suite's own diagnostic widths were never read), one resolver
(`resolve_gnn_widths`, arch-refused first), the writer refuses a stamp whose config claims another shape and a
resume whose launch moves it; (1b) contract v36 — `GnnArchV2SoftPolicy` behind `identity.arch_kind` (V2's trunk
and heads + `aux_policy_head`; `forward_batch`/`forward_single` serve V2's outputs EXACTLY, only the trainer's
`forward_batch_heads` evaluates the head), `model.aux_soft_policy {target_temperature, weight}` paired with the kind
by the schema and the trainer, the soft target with the temperature on the explicit entries and the tail carried
(`losses.soft_policy_target`; the 45k-ring measurement: α median 0.000 / p90 0.020, the whole-set `target^(1/4)`
put a median 40 % of the soft mass on the tail, the explicit-only form reads KL(t‖soft) 0.27 nats median with 76 %
of rows > 0.1, the admission line held), the LAW-18 rows `aux_soft_policy_loss` / `aux_soft_policy_kl_hard_vs_soft`
/ `policy_head_grad_norm` / `aux_policy_head_grad_norm` with the LAW-07 planted break (a construction returning the
hard target reads KL 0), `load_from_bc` admits a `reinit` head absent from the source; (2) contract v37 —
`train.heldout_gap` (a sha-pinned FROZEN slice loaded into its own engine buffer at the run's geometry, re-seeded
before every read, `run_declared_eval_step` every `interval` steps, `heldout_gap` on the stream against the train
mean since the last read; the freeze's planted break: an un-seeded read drifts), `ring_audit` gains
`one_hot_share_full_mr<k>` and `tail_only_full` (the 45k ring reads 0.2705 / 0.3502 / 0.1886 / 0 — PROBE-1's
numbers exactly); (3) CARD-SERVER-OWNED-COPY — `served_copy` in the pool, `Trainer.actor_state_dict` (the learner's,
always) for ActorSync and `inference_state_dict` / `deploy_module` (the EMA shadow when on) for the gate's candidate
and promotion, `ema_state` on the envelope with `deploy_state` for the frontier cell and the ladder bot (`weights:
ema` on the receipt), restored on resume (`ema_shadow_reseeded` from a shadowless stamp), the B-1 refusal deleted so
`train.ema.enabled: true` mints, the `actor_lag` row RETURNED to the manifest's REQUIRED set (gate 12 rc 0 on every
production config); (4+5) `configs/run10.yaml` — the twelve leaves (`run_id`, the kind, `reinit: [aux_policy_head]`,
T 4 / weight 4, `scheduler_t_max` 108 000 / `eta_min` 1e-4, the held-out block over
`run8_00045000_3bdedf76.ckpt.ring.bin` sha `2a3c4e57…` at 12 batches / seed 20260921 / interval 3 000), the LR
pinned through the trainer's scheduler (1e-3 / 5.5e-4 / 1e-4 at 0 / 54k / 108k; torch's cosine is PERIODIC past
the horizon, +< 1e-6 over the cell's ≈ 1 500 steps, the STOP LAW the guard), every by-name production pin names
run10 (the P6b count 7, the plant stem run11, `_MINTED_ARCH_KIND_ROW`); `RUN10_PREREG_2026-09-21.md` with the
eight-band block (PARSES; the parent's 45k ring reads the seven bands PASS and `replay_ratio` 3.03 MISS, as run9's
block read it). **THE THREE PROBLEMS THE PACKET DID NOT PRICE, stated rather than routed around:** (i) the 6×192
size conditional cannot warm-start from the 4×128 parent (no tensor shape in common; `load_from_bc` strict) — the
leaves and the bench path landed, the arming waits on a parent ruling (CARD-RUN10-SIZE-PARENT); (ii) the `model`
section did not exist — created, with `model.gnn` a BLOCK because the schema's arch partition admits blocks only;
(iii) the soft target's whole-set construction is an artefact in the sparse-row regime — the explicit-only form is
built, both numbers in the prereg. **The weight envelope's CPU pre-read** (prereg §1b/§8): the 45k parent
warm-started onto the new kind through the production seam, trained on its own ring at run10's rows on this CPU —
the fresh head's CE falls 5.7 → 2.5 over 60 steps and its own gradient norm 12.8 → 1.6, the ratio to the main head
24 → 3.2 and still falling — NOT SETTLED at 60 steps (a plateau at ≈ 3 would re-pick the weight at 2); the
...
CARD-E1-RULER-R6 carries the qualifier; CARD-RUN9-QUEUE moved. `configs/run9.yaml` stayed as minted until R367 §0(4) deleted it on 2026-09-21 (item 19). **Exit:** `make gates.exit` (`run_all.sh --with-slow`) on the
...
(19) R367 (2026-09-21, dev only; 0 box-h — no box exists): the register at R367 (next R368), R365's A1 (the release
was a priced act ruled by the architect; R367(d) is the rule), R366's Status amended (the 6×192 conditional WITHDRAWN,
the soft target / envelope / witness RATIFIED); CLAUDE.md gains R10 (the design standard + the review gate) and R11
(the price law); repo_design §8 amended; CARD-NET-EXPAND (run11: a function-preserving expansion, design before code)
and CARD-MECHANISM-SWEEP (the pre-existing runN / sha / `_Pool` inventory, applied on contact) opened, CARD-REVIEW-1
LANDED, CARD-RUN10-SIZE-PARENT closed. **REVIEW-1** (`7d9ec7dd`, a fresh read-only agent in its own worktree on
`51f40a18..19e8351d`): 34 findings — the mint commit had done the thing the standard forbids ("every by-name production
pin names run10", 11 edit sites + a run-named test), four things existed two or three times in `src/`, one compatibility
shim, one test affordance re-opening B-1, and two correctness gaps for box time (the STOP LAW as prose over a periodic
LR; the actor wiring unpinned). **THE FIX LEG, one commit per class:** F5 `a92feada` — `FlooredCosineAnnealingLR`
(cosine to `eta_min` at `T_max`, held there; the parent's state format so every stamp resumes; every cosine config's
rows swept monotone over its whole `max_train_steps`), the actor wiring pinned with DISTINCT actor/deploy sentinels
(a revert to `inference_state_dict` reds — driven), the `from_config` model-less shim deleted, `served_copy(model,
None)` a deep copy (a pool never serves its argument), `run.py`'s engine import at the top, the width refusal ahead
of the identity early return; F1 `95489040` — `mantis.config.census.production_configs` = `discover_configs` −
`EXEMPT_CONFIGS` (the exempt rows live there with grounds; `armed_aborts` holds neither tuple), gate 12 and the seven
former pin sets read it, `_MINTED_ARCH_KIND_ROW` / the per-mint count / the `run11` plant stem deleted, the by-name
binding tests (F-P2B / N-1) REVERSED and replaced by the census's own, `configs/run9.yaml` deleted with ZERO pin
edits (a `DISSOLVED_PATHS` row for gate 10); F4 `3d93447c` — `graph_collate.segment_ids/segment_sum/segment_softmax`
the ONE segment vocabulary (§c.6's declared duplication resolved), `losses.sparse_tail`/`rebuild_sparse_target` the
one tail (`_segment_probs`/`_hard_target` gone), every net exposes `forward_batch_heads` + `policy_heads()` so the
trainer calls one method, the kinds table and the `model.gnn` parser once in `mantis.config` (`model → config`),
...
discharged at HEAD; 0 must-fix, 8 should-fix, 13 minor of its own — landed in `f088407e`: the probe's fourth tail
copy, the trainer's two authorities for "this net has an aux head" (the NET is now the authority, the kinds table a
cross-check refused by name, pinned), the exempt set PINNED by name (an exemption is a ruling event — the one escape
the census leaves), five census consumers made path-faithful, the LR test swept once per DISTINCT schedule, two false
headers/comments and the prereg's periodic-tail / `PRODUCTION_CONFIGS` lines repaired in place. **Deferred with
grounds** (the report's §9, CARD-MECHANISM-SWEEP): F5.6 (the slice's buffer is a virtual reservation; the fix is a
Rust-bridge header reader), the flat head-width loop (the conformance suite feeds it), the nine literal full-config
dicts, the 21 `_Pool` fakes, the pre-existing runN inventory. **Numbers (this host, sandboxed default tier):** 5 047 passed / 9 skipped / 56 deselected (356 s);
range `19e8351d..f088407e` net code −600 lines with the review reports +1 000 docs. **Exit:** `make gates.exit`
(`run_all.sh --with-slow`) as a systemd user unit on `f088407e`: 21 gates GREEN incl. the slow tier, 2026-09-21 19:10–20:52 UTC; the default tier 5 047 passed / 9 skipped / 56 deselected (356 s); the slow tier
5 passed; the integration tier 49 passed / 2 skipped (3 755 s); collected 5 112 (floor 4 862). `dev` pushed to `origin/dev` by this session (the packet's push is the dispatcher's). NEXT: the box, on the operator's rent
(R367(d)/(e)), running the prereg's §6 with the admission bench in place of the withdrawn size bench.
