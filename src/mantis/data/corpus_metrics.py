...
class _NullProgress:
    """No-op stand-in for rich.Progress when rich is unavailable."""
...
def _progress_ctx() -> Iterator[Any]:
    """Yield a rich Progress (lazily imported) or a no-op fallback."""
    try:
...
def _elo_band(elo: int | None) -> str | None:
    if elo is None:
        return None
    for (lo, hi), label in zip(ELO_BANDS, ELO_LABELS, strict=False):
        if lo <= elo < hi:
            return label
    return ELO_LABELS[-1]
...
def analyse_opening_diversity(
    ...
    result: dict[str, float] = {
        f"unique_at_move_{cp}": len(unique_hashes[cp]) for cp in checkpoints
    }
...
def _manifest_elo_band(elo: int | None) -> str:
...
def _game_max_elo(r: GameRecord) -> int | None:
