"""Representation-aware autocast dtype resolver — the bf16-graph pin.

`resolve_amp_dtype` (mantis.config.resolve.amp) is now THE single decision authority
...
`train.amp_dtype` and `train.fp16` are DELETED, so the pin is the only authority.
...
LayerNorm -> NaN. bf16 keeps fp32's 8-bit exponent at 2-byte width, so the
...
def amp_dtype_for(representation: str) -> torch.dtype:
    """Autocast dtype for a representation kind (ONE authority).

    Args:
        representation: the identity representation the run declares.

    Returns:
        `torch.bfloat16`, the pin.
