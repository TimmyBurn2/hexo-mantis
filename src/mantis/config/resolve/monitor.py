...
`MonitorConfig`: that dataclass carries a code-side default for every field it holds, so
adding one here would be the second-authority shape R1 kills and would contradict R242's "no
default". (No tally is stated, deliberately — G-DFIX-4 / R192(e) derive-or-delete. An earlier
draft of this line said "a 28th one", transcribed from a design note; the live field count was
already past that on the day it was written, which is exactly how a transcribed number becomes
misinformation a reader trusts.) The other two are read by their OWN resolver:
