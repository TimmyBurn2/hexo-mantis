...
MANIFEST: tuple[ArmedAbort, ...] = (
    ...
    ArmedAbort(
        ...
        note=(
            ...
            "correcting the WITHDRAWN DR-8; RE-POINTED by R242/ADJ-D12): consec=3 counts "
            "consecutive CHECKS at a stride of monitor.gate_interval train steps — the knob "
            "was train.log_interval until R242 split arming from narration, and the ARITHMETIC "
            "BELOW IS UNCHANGED because every committed config mints gate_interval EQUAL to its "
            "own log_interval in that same bundle. So at run5's gate_interval 1000 the three "
            "samples still span 2000 steps. The earliest possible fire is nonetheless step "
            "25000, because SAMPLING IS NOT GATED BY min_step — the history accumulates from "
            "step 1000, holds 25 samples by step 25000, and min_step gates only the FIRE. DR-8's "
