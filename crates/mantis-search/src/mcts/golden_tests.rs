// Exceeds the 300-line soft cap: the full S1–S4 fixture roster + bit extractors
// + capture/assert harness stay in one file (reach mcts internals; see D1).
//! Completed-Q golden byte-identity harness for the completed-Q sites.
//!
...
//!   - S3 `gumbel.rs::score` (per-candidate, seeded RNG → deterministic)
//!   - S4 PUCT root Dirichlet noise (seeded RNG + Gamma → deterministic)
//! across the fixtures incl. 6 RED-TEAM adversarial ones.
//!
//! Goldens are frozen in `tests/fixtures/search_golden/completed_q/golden_bits.txt`.
//! Capture flow: run `test_capture_goldens_print` (ignored) → paste its stdout
//! into the golden file → the non-ignored `test_golden_*` asserts live bits ==
//! frozen bits (BIT-EXACT — an FMA→mul regression is sub-ULP but nonzero, so
//! `==` on bits catches it; `abs()<1e-6` would NOT).
//!
//! S3 is a regression guard ONLY: the completed-Q refactor must not touch
//! `gumbel.rs::score`.

// The seeded RNG constants (`0xD_F1_C0DE`, `0x5_4D_1_5EED`) are FROZEN — the
// goldens were captured with these exact seeds. Their underscore grouping is
// preserved verbatim; suppress the cosmetic `unusual_byte_groupings` lint.
#![allow(clippy::unusual_byte_groupings)]

use super::*;
use mantis_core::board::Board;
use crate::mcts::node::Node;
use crate::mcts::gumbel::GumbelSearchState;
...
const N_ACTIONS: usize = mantis_core::board::BOARD_SIZE * mantis_core::board::BOARD_SIZE + 1;
...
///
/// This pins the PUCT root-noise sequence f32::to_bits-identical. The
/// gumbel-arm Dirichlet deletion touches only the Gumbel branch in the
/// self-play loop; the PUCT arm calls the SAME `sample_dirichlet` +
/// `apply_dirichlet_to_root` exercised here. S4 holding byte-identical proves
/// the deletion cannot perturb the PUCT path.
...
fn test_golden_s1_s2_byte_identical() {
    for (key, live) in all_goldens() {
        ...
        assert_eq!(
            live, golden,
            "{key}: completed-Q output bits diverged from the frozen golden — \
             the refactor is NOT byte-pure on this fixture"
        );
    }
}
...
fn test_golden_s3_score_unchanged() {
    ...
    assert_eq!(
        live, golden,
        "S3_SCORE: gumbel.rs::score per-candidate bits changed — the completed-Q \
         refactor must NOT touch S3 (gumbel.rs untouched contract)"
    );
}
...
fn test_golden_s4_puct_dirichlet_unchanged() {
    ...
    assert_eq!(
        live, golden,
        "S4_PUCT_DIRICHLET: PUCT root Dirichlet noise bits changed — the \
