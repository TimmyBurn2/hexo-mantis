...
/// `forced_win_fired`).
#[allow(clippy::too_many_arguments)] // VERBATIM signature (ls + coord + weight + coverage + geometry)
...
mod o1_tests {
    ...
    fn test_one_hot_survives_aggregate_to_local() {
        ...
        // (WP6 port: direct field pokes → `Board::from_stones`; window_center is
        // identical between the old manual bbox (-1,5) and the stone bbox (0,4).)
...
mod ls_tests {
    ...
    ///
    /// WP6 port: direct field pokes → `Board::from_stones` (the stone bbox
    /// q∈[0,34] recomputes the exact manual bbox, so `window_center()==(17,0)`).
