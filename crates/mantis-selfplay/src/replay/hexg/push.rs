...
/// closes both at push.
pub fn validate_visit_prob(q: i16, r: i16, prob: f32) -> Result<(), String> {
...
/// Reject a non-finite outcome (value target) at push time, naming the value.
pub fn validate_outcome(outcome: f32) -> Result<(), String> {
...
impl HexgBuffer {
    ...
    pub fn push_record_impl(&mut self, rec: &GraphRecord, game_id: i64) -> Result<(), String> {
        ...
        // Validate every stone player before any mutation of `self`.
        for &(q, r, player) in &rec.stones {
            validate_stone_player(q, r, player)?;
        }
    ...
    /// Read record at logical slot `slot` back into a `GraphRecord` (exact
    /// inverse of `push_record_impl`).
    #[must_use]
    pub fn record_at(&self, slot: usize) -> GraphRecord {
