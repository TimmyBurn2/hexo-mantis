...
fn resize_preserves_records_at_a_non_default_capacity_across_wrap() {
    ...
    for i in 0..6 {
        let visits: Vec<(i16, i16, f32)> =
            (0..(129 + i)).map(|j| (j as i16, i as i16, 1.0 / (129 + i) as f32)).collect();
        let mut rec = sample_record();
        rec.visits = visits;
        rec.ply_index = i as u16;
        buf.push_record_impl(&rec, i as i64).unwrap();
    ...
    assert_ne!(buf.head, 0, "premise: head is mid-ring so resize must linearise");
    ...
    buf.push_record_impl(&GraphRecord { visits, ..sample_record() }, 99).unwrap();
