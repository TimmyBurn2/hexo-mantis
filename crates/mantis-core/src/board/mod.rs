// Exceeds the 300-line soft cap: board facade + the ported in-file unit and
// property suites live beside the module they pin (in-src tests near code).
...
//!
//! Legal moves: empty cells within hex_distance ≤ `legal_move_radius` of any
//! placed stone (default 5; the official rule's 8 is capped for self-play
//! extent).  On an empty board a 5×5 starting region is used (25 cells) to
//! bound the first move.
//!
...
mod tests {
    ...
    fn legal_move_radius_capped_at_5() {
        // Stone at (0,0). Every cell at hex_distance ≤ 5 must be legal;
...
mod prop_tests {
    ...
    proptest! {
        ...
        fn undo_restores_hash(indices in proptest::collection::vec(0usize..361, 1..=20usize)) {
            ...
            prop_assert_eq!(board.ply, Ply::ZERO, "undo must restore ply to 0");
