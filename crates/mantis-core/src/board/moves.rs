...
impl Board {
    ...
    pub fn legal_moves_set(&self) -> &FxHashSet<(i32, i32)> {
        ...
        // SAFETY: shared read; INV-2/INV-3 — no exclusive borrow can be created while this
        // reference is live (every dirty-true transition, hence every rebuild, requires `&mut self`).
                    ...
                    for i in 0..6i32 {
                        ...
                        match self.cells.get(&(q, r)).copied() {
                            ...
                            _ => {
                                dead = true;
                                break;
                            } // opponent or over-2-empties kill
                        }
                    }
...
mod tests {
    ...
    fn test_count_winning_moves_five_in_row() {
        ...
        board.apply_move(0, 9).unwrap();
        board.apply_move(0, 8).unwrap(); // P2 turn
        board.apply_move(1, 0).unwrap();
        board.apply_move(2, 0).unwrap(); // P1 turn
        board.apply_move(0, 7).unwrap();
        board.apply_move(0, 6).unwrap(); // P2 turn
        board.apply_move(3, 0).unwrap();
        board.apply_move(4, 0).unwrap(); // P1 turn
    ...
    fn test_count_winning_moves_five_blocked_one_end() {
        ...
        board.apply_move(-1, 0).unwrap();
        board.apply_move(0, 9).unwrap(); // P2 (blocker + filler)
        board.apply_move(1, 0).unwrap();
        board.apply_move(2, 0).unwrap(); // P1
        board.apply_move(0, 8).unwrap();
        board.apply_move(0, 7).unwrap(); // P2
        board.apply_move(3, 0).unwrap();
        board.apply_move(4, 0).unwrap(); // P1
    ...
    fn test_count_winning_moves_zero_when_early_game() {
        ...
        board.apply_move(4, 4).unwrap(); // P2
        board.apply_move(0, 5).unwrap();
        board.apply_move(5, 0).unwrap(); // P1
    ...
    fn test_has_player_long_run_detects_five_in_row() {
        ...
        board.mark_cache_dirty();
        assert!(
            board.has_player_long_run(Player::One, 5),
            "5 consecutive P1 stones should be detected"
        );
        assert!(
            !board.has_player_long_run(Player::Two, 5),
            "P2 has no long run"
        );
    }
    ...
    fn test_first_winning_move_returns_completing_cell() {
        ...
        assert!(
            b2.check_win(),
            "first_winning_move must complete 6, got {mv:?}"
        );
        assert_eq!(
            b.first_winning_move(Player::Two),
            None,
            "P2 has no winning move"
        );
    }
    ...
    fn test_forced_win_move_depth2_sets_up_within_turn_win() {
        ...
        assert_eq!(
            b.forced_win_move(1),
            None,
            "4-in-a-row has no immediate win"
        );
        ...
        assert!(
            b2.first_winning_move(Player::One).is_some(),
    ...
    fn test_forced_win_move_targets_side_to_move_only() {
        ...
        assert_eq!(
            b.forced_win_move(2),
            None,
            "only the side-to-move's wins count"
        );
    }
    ...
    fn threat_moves_empty_board_is_empty() {
        ...
        assert_eq!(
            ...
            board.threat_moves_ref(Player::One)
        );
        assert_eq!(
            board.threat_moves(Player::Two),
            board.threat_moves_ref(Player::Two)
        );
