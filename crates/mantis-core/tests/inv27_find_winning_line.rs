...
//! colony-classified game endings.
//!
//! Pre-fix mechanism (`board/moves.rs::find_winning_line`): the function only
//! scanned outward from `last_move`. When two stones are placed per turn
//! (compound-turn rule) the **first** move of a turn can complete the
...
//! fallback (`player_wins`), so a downstream terminal classifier labels the
//! game as colony (= "winner exists, no winning_line"). The threat-head
//! target column in the per-row aux buffer is then all-zero on every
//! position of that game — the threat head learns "no 6-line exists" and
//! gradient-suppresses the threat-detection circuit (predecessor forensic
//! record).
//!
//! (File renamed from the predecessor's "threat_target_colony" — that name
//! describes the downstream aux-buffer consumer, not this core invariant.)
//!
...
fn inv27_cell_2_winner_last_move_off_line_colony_classification() {
    ...
    // find_winning_line MUST also find the line — this is the pinned invariant.
