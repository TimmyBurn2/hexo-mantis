...
_REQUIRED = ("schema_version", "server", "game_id", "bot", "opponent", "side", "time_control", "rated",
             "sims_configured", "search", "moves", "rejections", "result", "plies", "plies_seen", "moves_full", "wall_sec",
             "think_ms_total", "started_utc", "finished_utc", "server_clock")
_BOT_REQUIRED = ("name", "backend", "net_hash", "display_name", "profile_id")
_MOVE_REQUIRED = ("request_id", "stones", "time_limit", "placements", "sims", "ms", "server_date")
...
class GameReceipt:
    ...
    def __init__(self, *, server: str, game_id: str, bot: Mapping[str, Any], opponent: Mapping[str, Any],
                 ...
                 search: Mapping[str, Any], started: float) -> None:
    ...
    def add_move(self, *, request_id: int | None, stones: int, time_limit: float | None,
        ...
        self.moves.append({"request_id": request_id, "stones": int(stones), "time_limit": time_limit,
                           ...
                           "ms": float(ms), "server_date": server_date})
    ...
    def finish(self, *, winner: str | None, reason: str, finished: float,
        ...
        body = {
            ...
            "sims_configured": self.sims_configured, "search": self.search, "moves": list(self.moves),
...
def validate_receipt(body: Mapping[str, Any]) -> None:
    """Refuse a body a reader could not trust (every field present, the net hash a sha256, two placements and an integer `sims` per move, an outcome from OUTCOMES). Raises: ReceiptError naming the first field that fails."""
